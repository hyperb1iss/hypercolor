...
use hypercolor_core::input::screen::consumer::{CaptureSourceId, PixelExtent, PixelRect};
use hypercolor_core::input::screen::implementer::{
    ...
    CaptureFrameProcessor, CaptureGeometry, CapturePixelFormat, CaptureRotation, CaptureStageKind,
    CaptureStorage, CpuCaptureStorage, PhysicalOrigin, RawCaptureSurface, SourceScale,
};
...
fn crop_origin_tracks_every_rotation_from_negative_desktop_coordinates() {
    ...
    for (rotation, expected) in [
        ...
        (CaptureRotation::Flipped, PhysicalOrigin { x: -97, y: -199 }),
        (
            CaptureRotation::Flipped90,
            PhysicalOrigin { x: -97, y: -197 },
        ),
        (
            CaptureRotation::Flipped180,
            PhysicalOrigin { x: -98, y: -197 },
        ),
        (
            CaptureRotation::Flipped270,
            PhysicalOrigin { x: -99, y: -198 },
        ),
    ] {
        let processed = CaptureFrameProcessor::default()
