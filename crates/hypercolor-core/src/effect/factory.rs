...
pub fn create_renderer_for_metadata(metadata: &EffectMetadata) -> Result<Box<dyn EffectRenderer>> {
    match &metadata.source {
        ...
        EffectSource::Html { .. } => create_html_renderer(metadata),
...
#[cfg(feature = "servo")]
fn create_html_renderer(_metadata: &EffectMetadata) -> Result<Box<dyn EffectRenderer>> {
    Ok(Box::new(super::servo_renderer::ServoRenderer::new()))
}
