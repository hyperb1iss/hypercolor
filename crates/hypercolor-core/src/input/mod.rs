...
pub use screen::{
    PickerPersistenceDecision, PickerSelectionSnapshot, ScreenBranchPublication,
    ScreenCaptureDemand, ScreenPublicationDemandSnapshot, SyntheticScreenPublisher,
    picker_persistence_decision, picker_selection_snapshot,
};
...
impl InputManager {
    ...
    /// The strictest native execution policy any registered screen source declares.
    #[must_use]
    pub fn screen_native_execution_policy(&self) -> screen::ScreenNativeExecutionPolicy {
        self.with_inner(|inner| inner.screen_native_execution_policy())
    }
...
impl InputManagerState {
    ...
    pub fn has_interaction_source(&self) -> bool {
        self.sources
            .iter()
            .any(|source| source.key() == ManagedSourceKey::Interaction)
    }
    ...
    pub fn screen_native_execution_policy(&self) -> screen::ScreenNativeExecutionPolicy {
        self.sources
            .iter()
            .filter_map(ManagedInputSource::as_screen)
            .map(ScreenSource::native_execution_policy)
            .max()
            .unwrap_or_default()
    }
    ...
    fn transition_screen_capture_demand(
        ...
        let prior_demands = self
            ...
            .collect::<Vec<_>>();
        let mut capacity_preparation = self.prepare_screen_capacity(0)?;
        let source_graph_generation = self.bump_source_graph_generation();
...
fn managed_source_capture_domains(key: ManagedSourceKey) -> (bool, bool, bool) {
    match key {
        ...
        ManagedSourceKey::Interaction => (false, false, true),
...
mod host_source_swap_tests {
    ...
    use super::{
        ...
        ManagedSourceKey, ManagedSourceRole, SourceRoleBinding, SourceState, SourceSwapConflict,
        SourceSwapTarget,
    };
    ...
    impl HostSource {
        ...
        fn browser(name: &'static str) -> Self {
            Self::new(name, Arc::new(AtomicBool::new(false)))
        }
    }
    ...
    fn browser_candidate_rejection_preserves_candidate_and_host_source() {
        ...
        let prepared = plan
            .prepare(&mut candidate)
            .expect("all manager interaction sources share the host key");
        assert!(prepared.has_replacement());
        assert_eq!(manager.source_names(), ["old-host"]);
        assert_eq!(manager.source_graph_generation(), initial_generation);
    }
