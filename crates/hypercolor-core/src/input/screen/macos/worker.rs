...
use super::BackendWorkerCommand;
#[cfg(feature = "macos-capture-fixtures")]
use super::InputSource;
use super::admission::prepare_macos_exact_runtime;
use super::publication::{capture_source_id, publish_frame};
use super::{
    Arc, AtomicBool, CaptureActivityHandle, CaptureSessionAuthority, MacosCaptureBackend,
    MacosCaptureControl, MacosExactPublicationShared, MacosExactRuntime, MacosFrameEvent,
    MacosFrameMailbox, MacosFrameStatus, MacosScreenRuntimeTelemetry, Ordering, PreparedWorker,
    ResourceState, ScreenPublicationHealth, ScreenPublicationHub, ScreenPublicationHubError,
    ScreenWorkerBinding, SourceSessionSlot, TopologyState, WORKER_WAIT, WorkerCommand,
...
pub(super) fn handle_worker_commands(
    ...
    while let Ok(command) = command_rx.try_recv() {
        match command {
            ...
            WorkerCommand::Stop => stop.store(true, Ordering::Release),
            #[cfg(feature = "macos-capture-fixtures")]
            WorkerCommand::Backend(BackendWorkerCommand::ReconfigureProcessing {
                calibration,
                completion,
            }) => {
