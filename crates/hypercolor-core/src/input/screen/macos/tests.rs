...
/// Headroom between reading the wall clock and the fixture's first stamp.
///
/// Binding happens at the real "now" while the fixture frames carry these
/// pre-computed instants, so the gap must absorb scheduler stalls on loaded
/// CI runners; 20ms lost that race on the Intel macOS lane.
fn fixture_clock_headroom() -> Duration {
...
fn rolled_back_worker_generation_is_not_reused() {
    ...
    let first_generation = first.authority().generation();
    drop(first);
    ...
    assert_eq!(second.authority().generation(), first_generation + 1);
}
...
fn retired_worker_cannot_republish_after_stop_returns() {
    ...
    input.shell.adapter.backend().worker_generation.set(1);
    ...
    {
        let mut activity = lock(input.shell.adapter.activity());
        activity
            .replace_fence_and_activate(MacosPublicationFence(1), 1)
            .expect("test worker owns the capture activity");
    }
    let stop = Arc::new(AtomicBool::new(false));
    let worker_stop = Arc::clone(&stop);
    let (release_tx, release_rx) = mpsc::sync_channel(0);
    ...
    assert!(activity.activate(1).is_err());
    drop(activity);
...
#[test]
fn removed_source_prepares_no_macos_runtime_and_commits_empty_plan() {
    let native_frame = frame();
    let source = source(&native_frame);
    let exact = MacosExactPublicationShared::default();
    let mut builder = ScreenPlanBuilder::new();
    exact.install_hub(builder.publication_hub());
    exact.install_test_source(Some(source.clone()));
    let resolved =
        resolve_macos_publication_branch(&source, &cpu_demand(ScreenProcessingProfile::default()))
            .expect("fixture demand resolves")
            .expect("configured source owns fixture demand");
    let mut runtimes = Vec::new();
    commit_cpu_runtime(&mut builder, &exact, &source, resolved, &mut runtimes);
    ...
    let mut preparing = builder
        ...
        .expect("removed macOS source plan prepares");
    let ticket = preparing
        .worker_ticket(&source.epoch.source_id)
        .expect("removed macOS source owns one worker ticket");
    let (token, runtime) = prepare_macos_exact_runtime(ticket, None, &exact)
        .expect("removed macOS source finalizes without runtime");
    assert!(runtime.is_none());
    assert!(token.exact_ledger().resources().is_empty());
    preparing
        .acknowledge(token)
        .expect("removed macOS token belongs to its plan");
    let armed = preparing
        .arm(builder.current().generation(), revision, graph)
        .unwrap_or_else(|failure| panic!("removed macOS plan arms: {}", failure.error()));
    assert!(
        armed
            .candidate_state()
            .runtime_binding(&source.epoch.source_id)
            .is_none()
    );
    let committed = builder
        .commit(armed, revision, graph)
        .unwrap_or_else(|failure| panic!("removed macOS plan commits: {}", failure.error()));
    assert!(committed.plan().branches().is_empty());
}
...
fn macos_publication_transition_is_deterministic_at_zero_midpoint_and_completion() {
    ...
    let started = Instant::now() + fixture_clock_headroom();
...
fn rgba16float_cpu_publication_matches_the_shared_scalar_oracle() {
    ...
    let captured_at = Instant::now() + fixture_clock_headroom();
...
#[test]
fn rejected_native_transactions_leave_adapter_settings_uncommitted() {
    let admission =
        ScreenByteAdmissionCoordinator::new(ScreenAdmissionCapacity::new(u64::MAX, u64::MAX));
    ...
    input
        ...
        .expect("baseline demand activates");

    let demand_revision = input.shell.adapter.settings().revision();
    fixture.reject_next_stream_request();
    let rejected_demand = ScreenCaptureDemand::active_with_policy(
        ScreenCaptureCadence::NativeRefresh,
        ScreenCursorPolicy::Include,
    );
    input
        .set_screen_capture_demand(rejected_demand)
        .expect_err("rejected native demand does not commit settings");
    assert_eq!(input.shell.adapter.settings().snapshot().demand, committed);
    assert_eq!(input.shell.adapter.settings().revision(), demand_revision);

    let config_revision = input.shell.adapter.settings().revision();
    let baseline = input.shell.adapter.settings().snapshot().config;
    let mut rejected_config = baseline.clone();
    rejected_config.target_fps = 30;
    rejected_config.grid_cols = baseline.grid_cols.saturating_add(1);
    fixture.reject_next_stream_request();
    input
        .reconfigure_screen_capture(&rejected_config)
        .expect_err("rejected native configuration does not commit settings");
    assert_eq!(input.shell.adapter.settings().snapshot().config, baseline);
    assert_eq!(input.shell.adapter.settings().revision(), config_revision);
}

#[test]
fn asynchronous_capture_reconfiguration_commits_settings_after_native_activation() {
    let admission =
        ScreenByteAdmissionCoordinator::new(ScreenAdmissionCapacity::new(u64::MAX, u64::MAX));
    ...
    let (mut input, fixture) = MacosScreenCaptureFixture::source(config.clone(), admission);
    input.start().expect("fixture source starts idle");
    input
    ...
    thread::scope(|scope| {
        ...
        assert_eq!(settings.snapshot().config, config);
    ...
    assert_eq!(settings.snapshot().config, next);
}
...
fn processing_reconfiguration_preserves_the_native_capture_runtime() {
    ...
    let worker_generation = input.shell.adapter.backend().worker_generation.get();
    let revision = input.screen_publication_resolution_revision();
    let settings_revision = input.shell.adapter.settings().revision();
    let mut config = input.shell.adapter.settings().snapshot().config;
    ...
    assert_eq!(
        input.shell.adapter.backend().worker_generation.get(),
        worker_generation
    );
    ...
    assert_eq!(
        input.shell.adapter.settings().revision(),
        settings_revision + 1
    );
    let resolved = input
...
fn invalid_processing_reconfiguration_preserves_the_active_profile() {
    ...
    let settings_revision = input.shell.adapter.settings().revision();
    let worker_generation = input.shell.adapter.backend().worker_generation.get();
    let previous = input.shell.adapter.settings().snapshot().config;
    let mut invalid = previous.clone();
    invalid.exposure_ev = f32::INFINITY;

    assert!(input.reconfigure_screen_processing(&invalid).is_err());
    assert_eq!(input.shell.adapter.settings().snapshot().config, previous);
    assert_eq!(input.shell.adapter.settings().revision(), settings_revision);
