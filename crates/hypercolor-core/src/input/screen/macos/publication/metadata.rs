...
impl MacosNativeTargetManifest {
    /// Derive the manifest for one resolved Metal-native descriptor.
    ///
    /// Public so native-source fixtures outside this crate can prepare the
    /// daemon's Metal target the way the production worker does.
    ///
    /// # Errors
    ///
    /// Returns an error when the descriptor's source lacks a nonzero Metal
    /// identity or generation.
    #[doc(hidden)]
    pub fn new(descriptor: &ResolvedScreenPublicationDescriptor) -> anyhow::Result<Self> {
