...
/// One platform-native named capture source offered for explicit selection.
///
/// Sound servers that expose richer identity than the portable device list
/// (PipeWire and PulseAudio monitors on Linux) surface their sources through
/// this shape; platforms without such a server return none.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NamedAudioSource {
...
/// Enumerate the platform sound server's named capture sources.
///
/// Returns an empty list on platforms whose only device inventory is the
/// portable one, so callers fall back to portable enumeration uniformly.
///
/// # Errors
///
/// Returns an error when the platform sound server exists but cannot be
/// queried.
pub fn enumerate_platform_named_audio_sources() -> anyhow::Result<Vec<NamedAudioSource>> {
    #[cfg(target_os = "linux")]
    {
        Ok(linux_audio::enumerate_named_audio_sources()?
            .into_iter()
            .map(|source| NamedAudioSource {
                id: source.id,
                name: source.name,
