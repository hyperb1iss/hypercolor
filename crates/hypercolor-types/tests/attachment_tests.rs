use hypercolor_types::attachment::{
    ...
    channel_name_matches_slot_alias, slot_id_matches_channel_name, slugify_slot_id,
};
...
fn slot_alias_matching_uses_default_profile_slug_rules() {
    ...
    assert_eq!(slugify_slot_id("Channel 01"), "channel-01");
    assert_eq!(slugify_slot_id("!!!"), "slot");
    assert!(slot_id_matches_channel_name("channel-1", "Channel 1"));
    assert!(slot_id_matches_channel_name("CHANNEL-01", "Channel 1"));
    assert!(channel_name_matches_slot_alias(
        Some("ATX Strimer"),
        Some("atx-strimer")
    ));
    assert!(channel_name_matches_slot_alias(
