...
fn execute_linux(args: &InstallReleaseArgs) -> Result<()> {
    ...
    let topology = InstallTopology::new(&args.install_prefix, &args.install_dir, &home)?;
...
#[cfg(test)]
mod tests {
    use super::{InstallTopology, require_bounded_absolute};
    use std::path::Path;

    #[test]
    fn install_topology_is_exact_and_systemd_safe() {
        let topology = InstallTopology::new(
