...
<key>EnvironmentVariables</key><dict><key>HYPERCOLOR_MACOS_OWNER</key><string>direct-launchd</string><key>HYPERCOLOR_LOG</key><string>info</string><key>PATH</key><string>/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin:{binary_directory}</string></dict>\n\
