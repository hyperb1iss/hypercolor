...
fn bootstrap_rejects_a_real_foreign_owner_despite_safe_mode() {
    ...
    let (metadata, path) = if root_metadata.is_owned_by_current_user() {
        ...
        (authority.metadata().expect("fixture metadata"), owned)
    } else {
        (root_metadata, Path::new("/").to_path_buf())
    };
    assert!(matches!(
        super::require_safe_bootstrap_directory(metadata, &path),
