...
pub type CliExtensionFuture<'a> = Pin<Box<dyn Future<Output = Result<()>> + 'a>>;
...
pub enum Commands {
    ...
    /// Dynamic driver and device controls
...
async fn execute_external_command(
    args: &[String],
    extensions: &[&dyn CliExtension],
    client: &DaemonClient,
    ctx: &OutputContext,
) -> Result<()> {
    let Some((command_name, command_args)) = args.split_first() else {
        bail!("missing external command");
    };

    for extension in extensions {
        if extension.command_name() == command_name {
            return extension.execute(command_args, client, ctx).await;
        }
    }
