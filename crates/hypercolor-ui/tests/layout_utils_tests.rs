...
fn sample_device_summary(name: &str, zones: Vec<api::ZoneSummary>) -> api::DeviceSummary {
    api::DeviceSummary {
        ...
        origin: hypercolor_types::device::DeviceOrigin::new(
            "fixture-driver",
            "test",
            hypercolor_types::device::DriverTransportKind::Usb,
        ),
        presentation: hypercolor_types::device::DriverPresentation {
            label: "Fixture Driver".to_owned(),
            short_label: Some("Fixture".to_owned()),
            accent_rgb: Some([128, 255, 234]),
            secondary_rgb: Some([255, 106, 193]),
            icon: None,
            default_device_class: Some(hypercolor_types::device::DeviceClassHint::Controller),
        },
