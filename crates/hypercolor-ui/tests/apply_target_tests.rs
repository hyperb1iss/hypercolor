use hypercolor_ui::apply_target::{ALL_LIGHT_ZONES_VALUE, ApplyTarget};
...
fn select_values_round_trip_through_apply_targets() {
    ...
    assert_eq!(
        ApplyTarget::from_select_value(ALL_LIGHT_ZONES_VALUE.to_owned()),
        ApplyTarget::AllLightZones,
    );
    assert_eq!(
    ...
    assert_eq!(
        ApplyTarget::AllLightZones.select_value(),
        ALL_LIGHT_ZONES_VALUE,
    );
    assert_eq!(
...
fn only_zone_targets_expose_zone_ids() {
    ...
    assert_eq!(ApplyTarget::AllLightZones.zone_id(), None);
    assert_eq!(
