...
use super::executor::{
    app_owner_stop_authority, hold_app_sidecar_supervisor, release_app_sidecar_supervisor,
    update_app_sidecar_gate_for_autostart,
};
use super::launchd::service_label;
use super::model::{MacosCaptureOwnerRestartOutcome, MacosDaemonOwnerRemedyOutcome};
use super::planning::{
...
use hypercolor_macos_owner::launchctl_service_disabled;
...
fn managed_topologies_resolve_only_their_exact_stop_authority() {
    assert_eq!(
        app_owner_stop_authority(MacosDaemonOwner::AppSidecar),
        StopAuthority::SupervisedChild
    );
    assert_eq!(
        app_owner_stop_authority(MacosDaemonOwner::DirectLaunchd),
        StopAuthority::ServiceManager(ServiceIdentity::launchd_direct())
    );
    assert_eq!(
        app_owner_stop_authority(MacosDaemonOwner::Homebrew),
        StopAuthority::ServiceManager(ServiceIdentity::homebrew())
    );
    assert_eq!(
        app_owner_stop_authority(MacosDaemonOwner::Standalone),
...
fn capture_owner_restart_revalidates_and_publishes_a_new_epoch() {
    ...
    assert_eq!(
        ...
        MacosCaptureOwnerRestartOutcome::Restarted {
            owner: ServiceIdentity::launchd_direct(),
...
fn standalone_capture_owner_restart_returns_typed_user_remedy() {
    ...
    assert_eq!(
        ...
        MacosCaptureOwnerRestartOutcome::UserActionRequired {
            owner: ServiceIdentity::STANDALONE,
            owner_epoch: 1,
