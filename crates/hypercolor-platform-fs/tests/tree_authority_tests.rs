...
fn bounded_child_names_enforce_count_and_aggregate_byte_limits() {
    let count_directory = tempfile::Builder::new()
        .prefix("platform-fs-")
        .tempdir_in(env!("CARGO_MANIFEST_DIR"))
        .expect("temporary count directory");
    ...
    let bytes_directory = tempfile::Builder::new()
        .prefix("platform-fs-")
        .tempdir_in(env!("CARGO_MANIFEST_DIR"))
        .expect("temporary byte directory");
...
fn metadata_and_regular_open_reject_hardlinks_and_special_files() {
    ...
    create_socket_entry(&directory.path().join("socket"));
...
fn cleanup_rejects_hardlinks_and_special_members() {
    ...
    create_socket_entry(&directory.path().join(".hypercolor-stage-special/socket"));
    special
        .remove()
