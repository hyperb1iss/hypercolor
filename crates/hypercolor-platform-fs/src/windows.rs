...
pub(super) fn durable_replace(source: &Path, destination: &Path) -> io::Result<()> {
    // MoveFileExW accepts a directory source over a regular destination on
    // some volumes; the replacement contract is file semantics on every
    // platform, so refuse a directory source the way the Unix arm does.
    if fs::symlink_metadata(source)?.is_dir() {
        return Err(io::Error::new(
            io::ErrorKind::InvalidInput,
            "directory source cannot replace a regular destination",
        ));
    }
