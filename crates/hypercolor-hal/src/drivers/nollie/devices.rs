...
const fn nollie_hid_intent(interface: u8, max_report_len: usize) -> TransportIntent {
...
macro_rules! nollie_descriptor {
    (
        ...
        DeviceDescriptor {
            ...
            name: $name,
            family: $family,
            transport: resolve_current_transport(nollie_hid_intent($interface, $max_report_len))
                .expect("Nollie HID transport should support the current platform"),
...
static NOLLIE_DESCRIPTORS: LazyLock<Vec<DeviceDescriptor>> = LazyLock::new(|| {
    vec![
        nollie_descriptor!(
            vid: PRISM_VENDOR_ID,
            pid: PID_PRISM_8,
