...
impl<C> LoopSender<C> {
    ...
    /// # Errors
    ///
    /// Returns the original command when the transport can no longer accept
    /// it. The PipeWire loop channel does not observe a dropped receiver,
    /// so on Linux a command sent to a loop that already exited is accepted
    /// and discarded with the queue; callers fence loop lifetime through the
    /// capture session state rather than through this result.
...
mod tests {
    ...
    // The PipeWire loop channel accepts sends after its receiver is gone, so
    // only the portable transport can prove the rejected command round-trips.
    #[cfg(not(target_os = "linux"))]
    #[test]
