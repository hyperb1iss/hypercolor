...
//! are the fence, and they are deliberately textual — the type system
//! cannot express "no other error factory exists".

use std::path::{Path, PathBuf};

/// Every `.rs` file under `src/api`, as `(relative path, source)`.
fn api_sources() -> Vec<(String, String)> {
    let api = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/api");
    let mut sources = Vec::new();
    let mut pending: Vec<PathBuf> = vec![api.clone()];
    while let Some(dir) = pending.pop() {
        for entry in std::fs::read_dir(&dir).expect("api directory reads") {
            let path = entry.expect("api entry reads").path();
            if path.is_dir() {
                pending.push(path);
                continue;
            }
            ...
            let relative = path
                .strip_prefix(&api)
                .expect("paths are under src/api")
                .to_string_lossy()
                .replace('\\', "/");
            let source = std::fs::read_to_string(&path).expect("api source reads");
...
fn is_response_type(error_type: &str) -> bool {
    let stripped = error_type
        ...
        .trim();
    matches!(stripped, "Response" | "axum::response::Response")
}
...
fn no_api_helper_returns_a_response_through_a_result() {
    ...
    for (relative, source) in api_sources() {
    ...
    assert!(
        offenders.is_empty(),
        "`Result<_, Response>` is extinct in src/api — these hand a built \
         Response back instead of a DomainError:\n{}",
        offenders.join("\n")
    );
}
