...
///
/// `drivers` entries carry credentials written by driver pairing flows and
/// the flattened `extensions` sections belong to out-of-tree crates that
/// share this file; neither is recoverable once a save drops it. The copy
/// has to be explicit because normalization only inserts missing defaults,
/// so it can seed a driver entry but never reconstruct its settings.
