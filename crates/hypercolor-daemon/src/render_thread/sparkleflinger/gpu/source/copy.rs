...
use super::super::super::{CompositionAdjust, CompositionPlan, CompositionTransform};
use super::super::{
    COMPOSE_WORKGROUP_HEIGHT, COMPOSE_WORKGROUP_WIDTH, GpuCompositorPipeline,
    GpuCompositorSurfaceSet, GpuCompositorTexture, PendingUploadBuffers, SOURCE_COPY_PARAM_BYTES,
    texture_extent,
};
use super::{
    CachedCpuSourceStorage, CachedReadbackAdjust, CachedReadbackKey, CachedReadbackLayer,
    CachedReadbackTransform, CachedSourceCopyBindGroup, CachedSourceUpload,
    GpuDisplaySourceTexture, GpuSourceFrame, SOURCE_COPY_BIND_GROUP_CACHE_CAP,
    SourceCopyBindGroupCache, gpu_source_frame,
};
...
pub(in crate::render_thread::sparkleflinger::gpu) fn prepare_display_source_texture(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
...
pub(in crate::render_thread::sparkleflinger::gpu) fn upload_frame_into_source_texture(
    device: &wgpu::Device,
    encoder: &mut wgpu::CommandEncoder,
...
pub(in crate::render_thread::sparkleflinger::gpu) fn copy_frame_into_output_texture(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
    pipeline: &mut GpuCompositorPipeline,
...
pub(in crate::render_thread::sparkleflinger::gpu) fn copy_gpu_source_frame_into_texture(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
    pipeline: &mut GpuCompositorPipeline,
    encoder: &mut wgpu::CommandEncoder,
...
pub(in crate::render_thread::sparkleflinger::gpu) fn write_rgba_texture(
    queue: &wgpu::Queue,
...
pub(in crate::render_thread::sparkleflinger::gpu) fn upload_frame_into_cached_texture(
    queue: &wgpu::Queue,
    texture: &wgpu::Texture,
    cached_upload: &mut Option<CachedSourceUpload>,
    frame: &ProducerFrame,
...
fn upload_frame_into_texture_with_encoder(
    ...
    let contents: Cow<'_, [u8]> = if padded_bytes_per_row == bytes_per_row {
        Cow::Borrowed(rgba_bytes)
    } else {
...
pub(in crate::render_thread::sparkleflinger::gpu) fn cached_source_upload(
    frame: &ProducerFrame,
) -> Option<CachedSourceUpload> {
    match frame {
...
pub(in crate::render_thread::sparkleflinger::gpu) fn cached_readback_key(
    plan: &CompositionPlan,
) -> Option<CachedReadbackKey> {
...
fn create_source_copy_bind_group(
    ...
    device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            ...
            wgpu::BindGroupEntry {
                binding: 2,
                resource: pipeline.source_copy_params.binding(),
            },
        ],
    })
}

/// Uploads source-copy params into the uniform ring and returns the dynamic
/// offset the dispatch must bind.
fn encode_source_copy_params_upload(
    ...
    pipeline
        .source_copy_params
        .write(device, queue, encoder, pending_upload_buffers, params)
        .offset
}
