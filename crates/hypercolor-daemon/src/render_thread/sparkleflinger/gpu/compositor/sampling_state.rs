...
impl GpuSparkleFlinger {
    ...
    fn latch_sampling_surface_readback(
        ...
        self.stage_sampling_surface_readback(width, height, encoder, native_screen_leases)?;
        Ok(match latched {
    ...
    fn stage_sampling_surface_readback(
        ...
        height: u32,
        encoder: Option<wgpu::CommandEncoder>,
        mut native_screen_leases: Vec<NativeScreenTextureLease>,
    ) -> Result<()> {
        ...
        {
            self.submit_sampling_encoder(encoder, native_screen_leases);
            return Ok(());
        }
