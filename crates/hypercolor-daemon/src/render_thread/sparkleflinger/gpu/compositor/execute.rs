...
use super::super::preview::{CachedPreviewSurfaceKey, preview_request_matches_plan};
use super::super::screen_upload::ScreenUploadPoolSaturated;
use super::super::source::{cached_readback_key, copy_frame_into_output_texture};
use super::super::{GpuCompositorOutputSurface, GpuSparkleFlinger};
use super::layers::{compose_layer_into_gpu, return_screen_frame_scratch, upload_screen_layers};
use super::{has_screen_upload_layers, screen_upload_content_keys};
use crate::render_thread::sparkleflinger::{
    ComposedFrameSet, CompositionPlan, PreviewSurfaceRequest,
};
...
impl GpuSparkleFlinger {
    ...
    ) -> super::super::GpuComposeOutcome {
        match self.compose(plan, requires_cpu_sampling_canvas, preview_surface_request) {
            Ok(composed) => super::super::GpuComposeOutcome::Produced(composed),
            Err(error) if error.is::<ScreenUploadPoolSaturated>() => {
                ...
                super::super::GpuComposeOutcome::Retained(gpu_composed_without_surfaces())
            }
            Err(error) => super::super::GpuComposeOutcome::Failed(error),
        }
    }
    ...
    pub(crate) fn compose(
        ...
        if requires_cpu_sampling_canvas && readback_key.is_none() {
            ...
            if super::super::preview::preview_requires_scale(request, plan.width, plan.height) {
