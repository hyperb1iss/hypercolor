...
use crate::render_thread::producer_queue::{NativeScreenTextureLease, SubmissionRetirementQueue};
...
use hypercolor_types::canvas::{BYTES_PER_PIXEL, PublishedSurface, SurfaceStateCounts};
...
use native_screen::NativeScreenBridge;
pub(crate) use native_screen::NativeScreenCopyOutcome;
use pipeline::GpuCompositorPipeline;
#[cfg(test)]
use preview::PendingPreviewReadback;
use preview::{CachedPreviewSurface, GpuPreviewSurfaceSet, PendingPreviewMap};
...
#[allow(
    dead_code,
    reason = "allocated by the platform screen bridges that publish execution targets"
)]
...
pub(crate) struct GpuSparkleFlinger {
    ...
    /// The platform's native screen execution path, if this device has one.
    screen_bridge: Option<Box<dyn NativeScreenBridge>>,
    /// The execution target the screen bridge currently publishes.
    screen_target: Option<ScreenNativeExecutionTarget>,
    native_screen_lease_retirements:
        SubmissionRetirementQueue<wgpu::SubmissionIndex, NativeScreenTextureLease>,
    #[cfg(test)]
