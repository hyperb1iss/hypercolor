...
use hypercolor_core::input::screen::implementer::{
    ...
    PhysicalOrigin, PlatformGpuApi, ScreenBranchPayload, ScreenPublicationColorimetry,
...
use hypercolor_core::input::screen::planner::{
    ...
    ScreenPublicationKind, ScreenPublicationRequest, ScreenResourceApi, ScreenResourceLifetime,
    ScreenSourceReflection, ScreenSourceSelector, ScreenUpscalePolicy, ScreenWorkerBinding,
    ScreenWorkerBindingState, ScreenWorkerPreparation, ScreenWorkerPreparationTicket,
    ScreenWorkerRetirement,
};
use hypercolor_core::input::screen::planner::{
    ScreenBranchLease, ScreenNativeExecutionPolicy, ScreenNativeExecutionUnavailableReason,
...
fn passive_screen_demand_without_renderer_target_has_no_execution_branch() {
    let demands = InputPublicationDemandHandle::new(ScreenNativeExecutionPolicy::Required);
...
#[test]
fn passive_screen_demand_binds_to_exact_cpu_when_native_is_preferred() {
    ...
    assert_eq!(demands.requested_hz(SourceKind::Screen), 60);
...
fn passive_screen_demand_binds_to_the_authoritative_renderer_target() {
    ...
    let _authority = demands.register(
        InputPublicationConsumer::Authoritative,
        InputPublicationDemand::default().with_screen_renderer_target(Some(&target)),
    );
...
fn incompatible_screen_demands_remain_exact_and_never_form_an_envelope() {
    ...
    assert_eq!(
        branch_extent(&shrunk.screen_branches[0]),
        extent(1_920, 2_160)
    );
    assert!(shrunk.capture_demand().is_active(CaptureDomain::Screen));
    drop(portrait);
    assert!(demands.snapshot().screen_branches.is_empty());
    assert!(
        !demands
            .snapshot()
            .capture_demand()
            .is_active(CaptureDomain::Screen)
    );
}
...
fn one_registration_preserves_every_screen_request_shape() {
    ...
    assert_eq!(exact.branches().as_ref(), branches.as_ref());
}
...
async fn pump_replans_exact_extent_changes_without_a_new_capture_transition() {
    ...
    wait_for_exact_extent(&publications, extent(1_280, 720)).await;
    let observed = transitions
        .lock()
        .expect("screen demand transitions remain readable")
        .clone();
    assert_eq!(
        observed.iter().filter(|demand| demand.is_active()).count(),
        1,
        "branch geometry changes re-plan the exact plan without a worker restart"
    );
    assert_eq!(observed.last(), Some(&ScreenCaptureDemand::active()));
...
#[tokio::test]
async fn reader_applies_required_execution_policy_to_passive_cpu_surface() {
    let lease = passive_cpu_surface_observation(ScreenNativeExecutionPolicy::Required).await;
    assert!(
        lease.is_none(),
        "missing native target must not borrow a passive CPU publication"
    );
}

#[tokio::test]
async fn reader_applies_preferred_execution_policy_to_passive_cpu_surface() {
    let (lease, pixels) = passive_cpu_surface_observation(ScreenNativeExecutionPolicy::Preferred)
        .await
    ...
    let retained_surface =
        retained_surface.expect("exact publications carry one stable CPU surface");
    assert_eq!(retained_surface.rgba_bytes(), pixels.as_slice());
    assert_eq!(canvas.as_rgba_bytes(), pixels);
    assert_eq!(
        canvas.storage_identity(),
        retained_surface.storage_identity()
    );
}

/// Publish one passive exact CPU surface and observe it through a reader
/// bound to `policy`, returning the lease (if any) and the published pixels.
async fn passive_cpu_surface_observation(
    policy: ScreenNativeExecutionPolicy,
) -> Option<(ScreenBranchLease, Vec<u8>)> {
    let transitions = Arc::new(StdMutex::new(Vec::new()));
    let source = ScreenDemandSource::new(Arc::clone(&transitions));
...
async fn missing_native_authority_reaches_source_alongside_passive_cpu_preview() {
    ...
    let authoritative = demands.register(
        InputPublicationConsumer::Authoritative,
        InputPublicationDemand::default().with_screen_renderer_execution(
...
async fn failed_exact_replacement_preserves_retirement_barrier_across_demand_changes() {
    ...
    assert_eq!(
        transitions
            ...
            .expect("screen demand transitions remain readable")
            .last(),
        Some(&ScreenCaptureDemand::active())
    );
    tokio::time::sleep(Duration::from_millis(25)).await;
    assert_eq!(publications.committed_state().branch_count(), 0);
...
async fn pump_rejects_a_superseded_demand_while_lifecycle_is_busy() {
    ...
    assert_eq!(
        transitions
            ...
            .count(),
        1,
        "the stale registration never reached the source"
    );
