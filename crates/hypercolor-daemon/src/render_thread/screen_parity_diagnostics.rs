//! Live screen parity snapshots served from the render thread.
//!
//! A diagnostic asks the renderer for the frame it is actually sampling:
//! the active screen publication copied through the native screen bridge,
//! read back as RGBA8, and sampled into zone colors with the live spatial
//! engine. The request travels over a one-slot mailbox the frame loop
//! services between frames, so the snapshot is exactly what the lighting
//! feed saw.
// The only consumer today is the macOS parity diagnostic endpoint; other
// targets compile the mailbox so the render thread stays target-neutral.
#![allow(
    dead_code,
    reason = "consumed by platform diagnostic endpoints; targets without one carry the seam unused"
)]
...
use hypercolor_core::input::screen::implementer::{
    CapturePixelFormat, ScreenBranchPublication, ScreenPublicationFreshness,
    ScreenPublicationHealth,
};
use hypercolor_core::input::screen::planner::ResolvedScreenPublicationDescriptor;
...
#[derive(Clone)]
pub(crate) struct ScreenParityDiagnosticHandle {
    sender: tokio_mpsc::Sender<ScreenParityRequest>,
}

pub(crate) struct ScreenParityDiagnosticMailbox {
    receiver: tokio_mpsc::Receiver<ScreenParityRequest>,
}

struct ScreenParityRequest {
    response:
        oneshot::Sender<std::result::Result<ScreenParityLiveSnapshot, ScreenParitySnapshotError>>,
}

pub(crate) struct ScreenParityLiveSnapshot {
...
#[derive(Debug, Error)]
pub(crate) enum ScreenParitySnapshotError {
...
pub(crate) fn screen_parity_diagnostic_channel()
-> (ScreenParityDiagnosticHandle, ScreenParityDiagnosticMailbox) {
    let (sender, receiver) = tokio_mpsc::channel(REQUEST_CAPACITY);
    (
        ScreenParityDiagnosticHandle { sender },
        ScreenParityDiagnosticMailbox { receiver },
    )
}

impl ScreenParityDiagnosticHandle {
    pub(crate) async fn snapshot(
        &self,
    ) -> std::result::Result<ScreenParityLiveSnapshot, ScreenParitySnapshotError> {
        let (response, receiver) = oneshot::channel();
        self.sender
            .send(ScreenParityRequest { response })
            .await
            .map_err(|_| ScreenParitySnapshotError::RendererStopped)?;
        receiver
            .await
            .map_err(|_| ScreenParitySnapshotError::RendererStopped)?
    }
...
impl ScreenParityDiagnosticMailbox {
...
fn capture_active_snapshot(
    ...
    let publication = publication
        .cloned()
        .ok_or(ScreenParitySnapshotError::NoActiveScreenPublication)?;
    let descriptor = descriptor
        .cloned()
        .ok_or(ScreenParitySnapshotError::NoActiveScreenPublication)?;
    if descriptor.source_epoch() != publication.source_epoch() {
        return Err(ScreenParitySnapshotError::PublicationIdentityChanged);
    }
    if descriptor.processing_profile().target_pixel_format() != CapturePixelFormat::Rgba8 {
        return Err(ScreenParitySnapshotError::UnsupportedOutputFormat);
    }
    ...
    {
        return Err(ScreenParitySnapshotError::NoActiveScreenPublication);
    }
    let frame = sparkleflinger
        .copy_screen_publication(&publication)
        .map_err(|_| ScreenParitySnapshotError::NativeReductionFailed)?
        .ok_or(ScreenParitySnapshotError::NativeReductionFailed)?;
    let rgba8 = read_rgba8(render_device, &frame)
        .map_err(|_| ScreenParitySnapshotError::SurfaceReadbackFailed)?;
    let zones = sparkleflinger
        .sample_texture_zone_plan(&frame, spatial_engine.sampling_plan().as_ref())
        .map_err(|_| ScreenParitySnapshotError::SpatialSamplingFailed)?
        .ok_or(ScreenParitySnapshotError::SamplingUnavailable)?;
    Ok(ScreenParityLiveSnapshot {
        publication,
        descriptor,
        width: rgba8.width,
        height: rgba8.height,
