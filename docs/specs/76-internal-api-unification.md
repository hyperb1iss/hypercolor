...
**Full reset** is defined as: defaults **plus an explicit copy of the current `drivers` map and flattened extension sections**, then normalize. Regression fixtures prove arbitrary driver settings and unknown extension documents survive reset round-trips. (Re-normalizing alone cannot reconstruct settings — normalization only inserts missing defaults.)
