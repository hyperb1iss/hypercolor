...
- **Native backends per platform.** D-Bus (logind, freedesktop screensaver) and kernel evdev on Linux, `WM_POWERBROADCAST` plus WTS session events on Windows, and NSWorkspace plus IOKit power notifications on macOS, all feeding the same `SessionEvent` stream.
