...
/// Size scales with canvas dims: ~1.17 MB at the default 640x480.
#[derive(Clone)]
pub struct Canvas {
...
///   frame times at legacy 320 by 200. GPU pixel readback via async staging buffer.
///
...
Thread 2: Screen Capture ──────────────────────────── OS thread, pinned
  ...
  - Downscale full resolution -> configured canvas dimensions
...
Threads 4..N: Servo Workers ───────────────────────── (if Servo is active)
  ...
  - Typically 2-4 threads, mostly idle at default canvas sizes

Tokio Runtime (multi-thread, 2-4 workers):
  - Axum web server + WebSocket streaming
  - Device backend I/O tasks (TCP/UDP/DTLS)
  - D-Bus service (zbus)
  - REST/WebSocket API traffic (TUI/CLI/Web UI connections)
  - mDNS discovery
...
1. **Canvas is RGBA and sized from configuration.** The default is 640 by 480, and live resize transactions retune the render loop at frame boundaries.
...
5. **The event bus (`watch` + `broadcast`) is platform-independent.** Frontends subscribe through WebSocket/MCP surfaces while daemon internals keep the same Rust channel semantics.
...
Group 1: Render target
  Binding 0: Texture 2D       -- output color attachment (configured RGBA canvas)
```
