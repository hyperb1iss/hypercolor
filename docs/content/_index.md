+++
title = "Hypercolor"
description = "Open-source RGB lighting orchestration for Linux, Windows, and macOS. Your desk is the canvas. Effects are web pages."
template = "index.html"
sort_by = "weight"
weight = 0
+++
...
Hue, Nanoleaf, Govee: driven from one process, on any of the three platforms. See the full
...
**Interactive effects.** Effects can react to your keyboard and mouse through a
consent-gated input pipeline that stays off until you enable it and only opens devices
while an interactive effect is running. See [input capture](@/guide/input-capture.md).
...
is Rust compiled to WASM via Leptos. Even the embedded browser is Servo. An adaptive render
loop with zero-copy frame encoding across 5 FPS tiers.
...
tints the UI edges to match the active effect, and the responsive shell works as a phone
remote for the rig.
