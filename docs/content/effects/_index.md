...
Effects are the visual core of Hypercolor. Each effect renders to a 320x200 pixel canvas at up to 60fps. The spatial engine then samples that canvas at each LED's physical position, translating pixels into colors that get pushed to hardware.

Hypercolor supports two rendering paths:

- **HTML/Canvas/WebGL effects** — authored in TypeScript using the `@hypercolor/sdk`, compiled to single-file HTML. This is the primary authoring path. Effects can use Canvas 2D for particle systems and procedural animation, or WebGL fragment shaders for GPU-accelerated noise, fractals, and mathematical visualizations.
