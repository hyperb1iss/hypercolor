...
- [Renderer internals](@/architecture/renderer-internals.md): Deep dive on the Servo HTML path, the native Rust path, the display-face variant, and the shipped GPU compositor lane.
...
The full lifecycle (timing budgets, snapshot mechanics, and the zone mapping contract) is detailed in [Render pipeline](@/architecture/render-pipeline.md).
...
The compositor's GPU lane has shipped: when `compositor_acceleration_mode` resolves to `gpu`, SparkleFlinger composition and spatial area sampling run as wgpu compute passes (see [Renderer internals](@/architecture/renderer-internals.md)). The native wgpu **shader-effect** renderer remains future work; treat any reference to a "native shader path" as referring to the compiled-in Rust effects, not GPU shaders.
