...
`Canvas` is a 2D RGBA pixel buffer in **sRGB gamma space**, backed by an `Arc<Vec<u8>>`. The default size is 640×480 (about 1.2 MB at 4 bytes per pixel). Coordinates are normalized `[0.0, 1.0]` throughout the pipeline, so effects are resolution-independent by design.
...
- **`EffectSource::Shader`** is **not runnable**: the factory bails with `shader effect '<name>' is not runnable yet`. A native wgpu/WGSL shader lane does not exist today. GLSL effects run via WebGL2 inside Servo rather than via wgpu; treat the native shader lane as future work.

There is no GPU **shader-effect** renderer today: the effect-renderer lane downgrades a GPU request to CPU. The compositor is a separate lane, and it does run on the GPU: when `compositor_acceleration_mode` resolves to `gpu`, scene composition and spatial area sampling execute as wgpu compute passes. See the [GPU compositor lane](@/architecture/renderer-internals.md) in renderer internals.
...
| `Area { radius }` | `(2r+1)²` reads on the CPU path; four summed-area-table lookups on the GPU path | Low-density zones spanning large canvas regions |
...
- **Broadcast channel** (capacity 256) carries discrete events: device connected, effect changed, scene activated. Every subscriber receives every event. Non-blocking; drops silently when the channel is full.
- **Watch channel** (latest-value semantics) carries high-frequency frame data, canvas previews, spectrum data, and device output. Subscribers always get the newest frame and skip stale ones automatically. The render thread never blocks waiting for a slow subscriber.
