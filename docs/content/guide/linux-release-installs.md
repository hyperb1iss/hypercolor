...
| The launcher changed since it was published | The service would start a program the installer never checked | Run `chmod -R u+w` on `releases/launcher`, remove it, remove the `hypercolor-*.service` companion units in `~/.config/systemd/user` that start it, and rerun; the install publishes both again |
| The service fails to start through the launcher | The launcher was published by a release whose CLI cannot launch, during an install that committed with the service stopped | Run `chmod -R u+w` on `releases/launcher`, remove it, remove the `hypercolor-*.service` companion units in `~/.config/systemd/user` that start it, and rerun the install; it publishes both from the new release |
