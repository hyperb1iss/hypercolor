...
The script asks for confirmation, then runs the installed
`hypercolor __uninstall-release`, which follows the locations the install
recorded rather than your current `XDG_*` variables. It removes:

- The systemd user service, after stopping and disabling it
- The command links in `~/.local/bin/`, the desktop launcher, app icons and
  shell completions the installer generated
- The installed releases (`~/.local/share/hypercolor/releases` by default)
- The update state (`~/.local/state/hypercolor/update` by default) and
...
Your data in `~/.local/share/hypercolor/` (scenes, layouts, credentials and
user effects) and your configuration at `~/.config/hypercolor/` are preserved.
The script prints the command to remove them if you want a full purge.

If the service, unit or a link was not generated by the installer, the
uninstall stops without changing anything and names what it found. See [How
Linux release installs are managed](@/guide/linux-release-installs.md#uninstall)
for the details.
...
| Linux | `~/.local/share/hypercolor/` | Scenes, layouts, credentials, user effects, logs, first-run marker |
