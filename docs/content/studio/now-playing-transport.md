...
The panel styles itself from the running effect. It pulls a palette off the live canvas and publishes it as `--np-primary`, `--np-secondary`, and `--np-tertiary` CSS variables, so the glow, the palette strip, and the zone swatches all tint to match what is on your LEDs right now. A state dot at the top left glows while the effect runs and dims when output is paused; beside it the panel names the running effect and its category.
...
- **Pause / Resume** is the center button. It is global: it pauses or resumes all device output at once, in every scene shape.
...
### The row toggles are per-zone, and they say so
...
The center transport Pause / Resume button is the exception: it is global. It pauses or resumes every device's output at once, and its tooltip says so (**Pause all output** / **Resume all output**). Use the per-zone row toggles when you want to stop one zone and leave the rest rendering.
...
The sidebar canvas thumbnail behaves differently depending on the page you are on. On pages that already have their own large preview, the sidebar drops the redundant thumbnail and instead drives the ambient palette. On `/`, `/effects`, and `/studio`, the Now Playing canvas runs in **Palette** mode: it extracts colors from the live frame to tint the chrome rather than drawing a second copy of the effect. Every other page (`/media`, `/devices`, `/settings`) shows the live thumbnail instead.
