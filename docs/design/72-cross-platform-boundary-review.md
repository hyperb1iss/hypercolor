...
Residue on 2026-08-22: the verify passes and `dirs` has one caller in
the four crates. `crates/hypercolor-tui/src/theme_picker.rs` still calls
`dirs::config_dir()` because the TUI depends on `hypercolor-types` only;
it stays until a types-level path helper exists. The Windows helper
binary derives `%LOCALAPPDATA%\hypercolor\helper-requests` on its own
(no workspace dependencies), documented as a pair with
`helper_client.rs`. The macOS owner store keeps `sync_parent_directory`
for attestation removal, which has no platform-fs primitive yet.
...
Residue on 2026-08-22: `core/src/input/screen/wayland.rs` is the last
in-core envelope author and stays until a Linux capture crate exists.
The app supervisor's `set_macos_external_owner` and
`set_macos_owner_offline` setters are daemon-ownership state, owned by
L1/L2, not source status. `daemon/src/api/macos_screen_parity.rs` still
downcasts to `MacosScreenshotReferenceCapture` inside a macOS-gated
module; moving that into a neutral RGBA helper in
`hypercolor-macos-capture` is optional polish.
...
Residue on 2026-08-22: L1 landed. `hypercolor_types::service` holds
`ServiceIdentity`, `DaemonRunMode`, `ServiceManager`, `ServiceStatus`,
and `StopAuthority`; every launcher declares
`HYPERCOLOR_SERVICE_IDENTITY` (systemd user and system units, the SCM
registry environment, the launchd plist, the Homebrew block, the CLI
templates, and the app's supervised child); the daemon corroborates the
claim per platform (launchctl pid identity and signed parent on macOS,
systemd `MainPID` and the supervised-parent pid on Linux, the SCM
dispatch and parent pid on Windows) and self-reports `service` in the
status API plus `service_identity_changed` on the bus. The legacy
`MacosDaemonOwner*Event`, `MacosCapabilityOwnerApi`, and UI and app
mirrors are deleted (spec 78 §0: no REST compatibility window; the
Python client and WS manifest were regenerated). The verify search now
finds only macOS adapters, the macOS TCC canary, the UI's macOS owner
choice bridge, and the capture-lane `MacosCapabilityOwner` labels
inside the input status surface, which name who holds TCC rather than
who launched the daemon and stay by design. Spec 76's prose now
describes `SystemStatus.service` and `service_identity_changed_v1` in
place of the deleted field and event.
...
Residue on 2026-08-22: L2 landed except the final gating item. The
app's `supervisor/plan.rs` is the pure payload-bearing `LauncherPlan`
with the `OwnerPreference` policy (Selected owners never reach
`SpawnChild`), systemd and the SCM fold into `LauncherProbe`, the
stopped-SCM `Start` arm exists (explicit fallback to a supervised child
when `sc.exe start` is refused, since a stopped registration cannot
collide), and macOS is a tenant: the verified external owner is the
probe, an offline owner is `Hold`, and the pending-standalone recovery
disposition is a decided `Hold`. `hypercolor_macos_owner::LaunchdAdapter`
is the one launchd adapter (bootstrap, bootout, kickstart, SIGTERM stop,
enable, disable, print); the CLI's plain verbs, the CLI coordinator,
the app executor, `uninstall.sh`, and `install-release.sh` speak through
it or its verbs, and the cask zaps `Hypercolor.plist`. `StopAuthority`
replaced the twin stop enums. Lifetime is kernel-backed: Linux
`PR_SET_PDEATHSIG` with the post-fork parent re-check in
`hypercolor-linux-session`, a macOS kqueue `EVFILT_PROC` watch in
`hypercolor-macos-owner`, Windows' job object; the daemon getppid
interval and the app's `EXTERNAL_OWNER_VERIFY_INTERVAL` are gone,
replaced by a `notify` watch on the owner store and one events socket
whose close is the session-loss edge.

Open: `hypercolor-macos-owner` is target-gated in the CLI only. The
daemon and the app still depend on it unconditionally because
`MacosDaemonSessionAttestation`, `MacosServerSessionId`, and
`MacosProtectedControlCredential` cross into `api/security.rs`,
`api/ws/session.rs`, and the app's `VerifiedDaemonConnection`. Gating
them is the neutral `VerifiedConnection { session_id, credential }`
vocabulary from §3 Supervision finding 4, which is its own package and
must serialize with the ws session lane. Cross-target `cargo check` is
blocked on macOS hosts by the `openssl-sys` and `libspa-sys` build
scripts; the Linux and Windows CI lanes prove the pdeathsig fixtures,
the systemd `MainPID` corroboration, the SCM dispatch corroboration,
and the `sysinfo` parent lookup on Windows.
...
Residue on 2026-08-22: every capture backend now compiles on every
target and `core/src/input/screen/**` carries zero `cfg(target_os)`
(the budget count fell from 50 files to 39). The adapter owns the
shared command endpoint and stop arm, the worker lifecycle
choreography, the owned-source record and CPU executor slot, the
two-phase settings adoption, and the `CaptureSourceShell` that every
platform source embeds; `CaptureBackendId` replaces `CapturePlatform`
and unifies `ScreenCaptureBackend`; and the module publishes the
consumer, planner, and implementer facades with the flat names kept one
release as `#[doc(hidden)]`. Four items stay open. Wayland's
`WORKER_DEMAND_*` word encodes a portal-unavailable parking state with
a rearm epoch that the versioned settings do not model, so it remains a
native lane rather than a mirror. The SPA chunk decoder
(`SpaChunkView`, `DoubleBuffer`, `decode_chunk`) is built on core's
frame vocabulary (`PixelRect`, `CaptureRotation`, `CaptureFrameError`,
`ScreenByteLease`), so it cannot move into `hypercolor-pipewire-interop`
until C1 relocates that vocabulary; it stays in `wayland.rs` as seam
code. `CaptureBackendId::current()` keeps its `target_os` arms because
the config registry validator in `hypercolor-types` has no backend in
hand; threading the composition root's identity through that registry
deletes the last capture cfg from the neutral crate. Wayland and
Windows are at 5077 and 4257 lines: the native halves (PipeWire format
negotiation and the analysis worker; DXGI pump, descriptor builders,
and GPU route preparation) are the remaining bulk and belong to C2's
interop quarantine, not to this adapter.
...
Residue on 2026-08-22: every screen consumer leases its own exact branch.
`FrameInput.screen` carries `Option<&Arc<ScreenBranchPublication>>`, so
native ScreenCast samples the published CPU surface, Servo retains the
Arc across queued frames, and LightScript projects surface publications
onto its grid as a documented box average while zone publications map
cell for cell; the WebSocket zones relay and the interactive preview
lease their branches from the hub. The compatibility mirror is gone on
the producer side as well: `ScreenCompatibilitySelection`, the demand
snapshot's mirror fields, the consumer-ranked single extent, and the
Windows and Wayland analyzer loops are deleted, and `ScreenCaptureDemand`
is extent-free (cadence and cursor only), with the worker lifecycle
derived from exact branch presence. The adapter's compatibility value
slot became `CaptureActivity`, a fenced epoch record. Three items stay
open. The reference CPU analyzer (`ScreenCaptureInput`, `ScreenData`,
`InputData::Screen`) survives as the fixture reference the macOS parity
tests compare against and as the unit under test for the analysis
suites; the remaining `rg` hits are those references. The analysis
capacity quoting API (`ScreenAnalysis*` compute types, the reserved-peak
parameter on `prepare_screen_capacity_plan`, and the daemon capacity
fields they fed) has nothing left to quote and should be retired with the
launcher lane's startup and API files. The Windows deterministic fixture
proves activation and fencing only; an exact-path Windows fixture that
pushes frames through `PreparedCpuPublicationFanout` would restore the
daemon-level pixel contract on Windows CI.
...
Measured on `nova/platform-boundaries` after the fourteen core packages
landed. Each goal-state item is checked with the command it names.

1. **Crate graph delta: met.** Added `hypercolor-worker-retention`,
   `hypercolor-gpu-frame`, `hypercolor-pipewire-interop`,
   `hypercolor-linux-session`, `hypercolor-windows-session`, plus the
   extension crates `hypercolor-linux-input`, `hypercolor-macos-media`,
   `hypercolor-macos-session`, and `hypercolor-windows-telemetry` (the
   WMI/PawnIO home that the cfg sweep needed because
   `hypercolor-windows-session` depends on core). `hypercolor-tray` is
   deleted. Every unconditional neutral-to-platform edge targets a crate
   that compiles everywhere through stubs (rule 4), with one residue: the
   daemon and the app still depend on `hypercolor-macos-owner`
   unconditionally until the `VerifiedConnection` vocabulary lands (L2
   residue above).
2. **cfg budget: met.** `rg -l 'cfg\(target_os' crates/hypercolor-core/src
   ...
   - The daemon composition roots and their tests are `api/config/live/capture.rs`,
     ...
     `startup/services.rs`, `startup/services/tests.rs`,
     `render_thread/gpu_device.rs`, `sparkleflinger/gpu/runtime.rs`, and
     `sparkleflinger/gpu/native_screen.rs` (the bridge selection point; its
     `mod` gates exist because the interop crates are target-gated deps).
   - The two pre-seam core files are `input/media.rs` and
     `input/audio/mod.rs`, both owned by E2.
3. **Platform nouns: met.** The status and trait greps find nothing.
   `hypercolor-types` carries `ServiceManager::WindowsScm` (a manager
   identity, not a platform mirror), the capture backend display labels,
   and the `MacosDaemonOwner*` event family is deleted. `CapturePlatform`
   became `CaptureBackendId` (DesktopDuplication, PipeWire,
   ScreenCaptureKit) with the `dxgi_desktop_duplication` wire name kept.
4. **GPU vocabulary: met.** One definition each in `hypercolor-gpu-frame`;
   the Servo render platform seam (`servo-context` feature) replaced the
   per-OS importer triplets in `effect/servo/gpu_import_backend.rs`.
5. **One engine per capability: met.** `ScreenCaptureAdapter<B>` with
   `CaptureSourceShell`, the shared endpoint, lifecycle choreography,
   settings rendezvous, and consumer/planner/implementer facades; one
   `host_fold`; `LauncherPlan` with `OwnerPreference` and `StopAuthority`
   over three tenants; one `LaunchdAdapter`; one session engine with
   Linux, Windows, and macOS backends; `hypercolor_platform_fs::user_dirs`
   behind `core/config/paths.rs`; `driver-api` credentials and the macOS
   owner store on `platform-fs` primitives.
6. **Conformance at the seam: met.** 51 fake-backend adapter tests, the
   platform fixture suites (macOS lib slice and Windows fixture tests now
   run in CI with `--no-tests=fail`), kqueue and pdeathsig lifetime
   fixtures, and the exact-screen suites from spec 73 wave 4 and T19.
7. **Platform N+1: met by construction.** Every capability is a seam
   implementation plus a composition-root arm; the audit found no
   remaining site where one platform's file names another platform.
8. **Specs and baselines: met.** Specs 73, 76, 77, and 78 moved with the
   code (78 is at rev 10 for `/media/authorize`); no FPS tier, cadence,
   resolution, output rate, pool size, or benchmark threshold decreased.
   The compatibility screen mirror is gone (117 `ScreenData` hits reduced
   to 28 fixture and reference uses).

Residue carried past this audit, each recorded in its package note:
the `VerifiedConnection` vocabulary and the daemon/app owner-crate gating;
the last in-core diagnostics envelope author in `wayland.rs` pending a
Linux capture crate; the SPA chunk decoder waiting on frame vocabulary
relocation; the analysis-capacity quoting API with nothing left to
quote; a Windows exact-path daemon pixel fixture; `hypercolor-tui`
resolving its theme path without core. Wave 4's E1 and E2 remain open by
design: E1 needs a signed-topology hardware probe that this lane could not
run, and E2 follows it. Linux and Windows CI lanes are the proof for the
platform code this macOS host could only compile by construction.
