...
- [ ] **Canvas resolution**: Effects MUST read `ctx.canvas.width/height` every frame — never hardcode. Effects ported from the legacy 320x200 SDK grid should use `scaleContext(ctx.canvas, { width: 320, height: 200 })` from `hypercolor`. The engine renders at 640x480 by default, configurable in Settings → Rendering.
