/** Nebula Drift — multi-layered space nebula with FBM noise and starfield */
export const NEBULA_SHADER = `#version 300 es
...
/** Plasma Engine — flowing plasma with theme-based color palettes */
export const PLASMA_SHADER = `#version 300 es
precision highp float;
...
void main() {
    ...
    plasma += sin(length(q - vec2(cos(time * 0.11) * 2.0, sin(time * 0.19) * 1.5)) * 1.4 + time * 0.37);
    plasma += sin(length(q + vec2(sin(time * 0.17) * 1.8, cos(time * 0.13) * 1.6)) * 1.9 - time * 0.49);
    plasma = plasma / 5.0;
