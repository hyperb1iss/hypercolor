...
if [ "$#" -eq 0 ]; then
  exec "$SCRIPT_DIR/servo-cache-build.sh" \
    cargo run -p hypercolor-daemon --bin hypercolor --profile preview --features servo -- --bind 127.0.0.1:9420
fi

exec "$SCRIPT_DIR/servo-cache-build.sh" \
  cargo run -p hypercolor-daemon --bin hypercolor --profile preview --features servo -- "$@"
