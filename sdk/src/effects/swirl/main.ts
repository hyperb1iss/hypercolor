import { canvas, color, combo, hexToRgb, num, type Rgb, rgbToHsl } from 'hypercolor'

const TAU = Math.PI * 2

const BLACK: Rgb = { b: 0, g: 0, r: 0 }

/** Whole degrees and whole percents, ready for a CSS `hsl()` string. */
function hexToHSL(hex: string): [number, number, number] {
    const { h, s, l } = rgbToHsl(hexToRgb(hex, BLACK))
