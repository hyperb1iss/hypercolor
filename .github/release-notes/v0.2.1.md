...
- **Razer** (70 supported devices via zerocopy `RazerReport`)
...
- **REST API** with typed envelope responses, OpenAPI spec generation (`/api/v1/openapi.json`), tiered auth (loopback read-only, API key for writes, network access controls), rate limiting, and CORS binding.
