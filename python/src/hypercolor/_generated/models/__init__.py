"""Contains all the data models used in inputs/outputs"""

from .action_confirmation import ActionConfirmation
from .action_confirmation_level import ActionConfirmationLevel
from .activate_playlist_response import ActivatePlaylistResponse
from .activate_playlist_response_200 import ActivatePlaylistResponse200
from .activate_scene_response import ActivateSceneResponse
from .activate_scene_response_200 import ActivateSceneResponse200
from .activated_scene_ref import ActivatedSceneRef
from .active_playlist_response import ActivePlaylistResponse
from .active_playlist_state_response import ActivePlaylistStateResponse
from .add_favorite_request import AddFavoriteRequest
from .add_favorite_response import AddFavoriteResponse
from .add_favorite_response_200 import AddFavoriteResponse200
from .api_error_body import ApiErrorBody
from .api_error_detail import ApiErrorDetail
from .applied_control_change import AppliedControlChange
from .applied_control_change_value import AppliedControlChangeValue
from .apply_control_changes_response import ApplyControlChangesResponse
from .apply_control_changes_response_values import ApplyControlChangesResponseValues
from .apply_control_surface_values_response_200 import (
    ApplyControlSurfaceValuesResponse200,
)
from .apply_effect_preset_response_200 import ApplyEffectPresetResponse200
from .apply_effect_request import ApplyEffectRequest
from .apply_effect_request_controls_type_0 import ApplyEffectRequestControlsType0
from .apply_effect_response import ApplyEffectResponse
from .apply_effect_response_200 import ApplyEffectResponse200
from .apply_impact_type_0 import ApplyImpactType0
from .apply_impact_type_1 import ApplyImpactType1
from .apply_impact_type_2 import ApplyImpactType2
from .apply_impact_type_3 import ApplyImpactType3
from .apply_impact_type_4 import ApplyImpactType4
from .apply_impact_type_5 import ApplyImpactType5
from .apply_impact_type_6 import ApplyImpactType6
from .apply_impact_type_7 import ApplyImpactType7
from .apply_layout_response import ApplyLayoutResponse
from .apply_layout_response_200 import ApplyLayoutResponse200
from .apply_layout_response_202 import ApplyLayoutResponse202
from .apply_policy_type_0 import ApplyPolicyType0
from .apply_policy_type_0_kind import ApplyPolicyType0Kind
from .apply_policy_type_1 import ApplyPolicyType1
from .apply_policy_type_1_kind import ApplyPolicyType1Kind
from .apply_policy_type_2 import ApplyPolicyType2
from .apply_policy_type_2_kind import ApplyPolicyType2Kind
from .apply_policy_type_3 import ApplyPolicyType3
from .apply_policy_type_3_kind import ApplyPolicyType3Kind
from .apply_policy_type_4 import ApplyPolicyType4
from .apply_policy_type_4_kind import ApplyPolicyType4Kind
from .asset_scan_status import AssetScanStatus
from .asset_update_request import AssetUpdateRequest
from .asset_upload_response import AssetUploadResponse
from .asset_warning_type_0 import AssetWarningType0
from .asset_warning_type_0_kind import AssetWarningType0Kind
from .asset_warning_type_1 import AssetWarningType1
from .asset_warning_type_1_kind import AssetWarningType1Kind
from .assign_live_zone_members_response_200 import AssignLiveZoneMembersResponse200
from .assign_members_request import AssignMembersRequest
from .audio_band import AudioBand
from .audio_device_info import AudioDeviceInfo
from .audio_devices_response import AudioDevicesResponse
from .authorize_input_monitoring_response_200 import AuthorizeInputMonitoringResponse200
from .authorize_screen_recording_response_200 import AuthorizeScreenRecordingResponse200
from .b_tree_map import BTreeMap
from .b_tree_map_additional_property_type_0 import BTreeMapAdditionalPropertyType0
from .b_tree_map_additional_property_type_0_kind import (
    BTreeMapAdditionalPropertyType0Kind,
)
from .b_tree_map_additional_property_type_1 import BTreeMapAdditionalPropertyType1
from .b_tree_map_additional_property_type_1_kind import (
    BTreeMapAdditionalPropertyType1Kind,
)
from .b_tree_map_additional_property_type_2 import BTreeMapAdditionalPropertyType2
from .b_tree_map_additional_property_type_2_kind import (
    BTreeMapAdditionalPropertyType2Kind,
)
from .b_tree_map_additional_property_type_3 import BTreeMapAdditionalPropertyType3
from .b_tree_map_additional_property_type_3_kind import (
    BTreeMapAdditionalPropertyType3Kind,
)
from .b_tree_map_additional_property_type_4 import BTreeMapAdditionalPropertyType4
from .b_tree_map_additional_property_type_4_kind import (
    BTreeMapAdditionalPropertyType4Kind,
)
from .b_tree_map_additional_property_type_5 import BTreeMapAdditionalPropertyType5
from .b_tree_map_additional_property_type_5_kind import (
    BTreeMapAdditionalPropertyType5Kind,
)
from .b_tree_map_additional_property_type_6 import BTreeMapAdditionalPropertyType6
from .b_tree_map_additional_property_type_6_kind import (
    BTreeMapAdditionalPropertyType6Kind,
)
from .b_tree_map_additional_property_type_7 import BTreeMapAdditionalPropertyType7
from .b_tree_map_additional_property_type_7_kind import (
    BTreeMapAdditionalPropertyType7Kind,
)
from .b_tree_map_additional_property_type_8 import BTreeMapAdditionalPropertyType8
from .b_tree_map_additional_property_type_8_kind import (
    BTreeMapAdditionalPropertyType8Kind,
)
from .b_tree_map_additional_property_type_9 import BTreeMapAdditionalPropertyType9
from .b_tree_map_additional_property_type_9_kind import (
    BTreeMapAdditionalPropertyType9Kind,
)
from .b_tree_map_additional_property_type_10 import BTreeMapAdditionalPropertyType10
from .b_tree_map_additional_property_type_10_kind import (
    BTreeMapAdditionalPropertyType10Kind,
)
from .b_tree_map_additional_property_type_11 import BTreeMapAdditionalPropertyType11
from .b_tree_map_additional_property_type_11_kind import (
    BTreeMapAdditionalPropertyType11Kind,
)
from .b_tree_map_additional_property_type_12 import BTreeMapAdditionalPropertyType12
from .b_tree_map_additional_property_type_12_kind import (
    BTreeMapAdditionalPropertyType12Kind,
)
from .b_tree_map_additional_property_type_13 import BTreeMapAdditionalPropertyType13
from .b_tree_map_additional_property_type_13_kind import (
    BTreeMapAdditionalPropertyType13Kind,
)
from .b_tree_map_additional_property_type_14 import BTreeMapAdditionalPropertyType14
from .b_tree_map_additional_property_type_14_kind import (
    BTreeMapAdditionalPropertyType14Kind,
)
from .b_tree_map_additional_property_type_15 import BTreeMapAdditionalPropertyType15
from .b_tree_map_additional_property_type_15_kind import (
    BTreeMapAdditionalPropertyType15Kind,
)
from .b_tree_map_additional_property_type_16 import BTreeMapAdditionalPropertyType16
from .b_tree_map_additional_property_type_16_kind import (
    BTreeMapAdditionalPropertyType16Kind,
)
from .b_tree_map_additional_property_type_17 import BTreeMapAdditionalPropertyType17
from .b_tree_map_additional_property_type_17_kind import (
    BTreeMapAdditionalPropertyType17Kind,
)
from .b_tree_map_additional_property_type_17_value import (
    BTreeMapAdditionalPropertyType17Value,
)
from .b_tree_map_additional_property_type_18 import BTreeMapAdditionalPropertyType18
from .b_tree_map_additional_property_type_18_kind import (
    BTreeMapAdditionalPropertyType18Kind,
)
from .binding_map import BindingMap
from .binding_source_type_0 import BindingSourceType0
from .binding_source_type_0_kind import BindingSourceType0Kind
from .binding_source_type_1 import BindingSourceType1
from .binding_source_type_1_kind import BindingSourceType1Kind
from .binding_source_type_2 import BindingSourceType2
from .binding_source_type_2_kind import BindingSourceType2Kind
from .binding_source_type_3 import BindingSourceType3
from .binding_source_type_3_kind import BindingSourceType3Kind
from .capture_authorization_response import CaptureAuthorizationResponse
from .capture_monitor import CaptureMonitor
from .capture_monitor_list_item import CaptureMonitorListItem
from .capture_picker_response import CapturePickerResponse
from .clear_scene_request import ClearSceneRequest
from .clear_scene_response_200 import ClearSceneResponse200
from .component_binding import ComponentBinding
from .component_binding_summary import ComponentBindingSummary
from .component_canvas_size import ComponentCanvasSize
from .component_category_type_0 import ComponentCategoryType0
from .component_category_type_1 import ComponentCategoryType1
from .component_category_type_2 import ComponentCategoryType2
from .component_category_type_3 import ComponentCategoryType3
from .component_category_type_4 import ComponentCategoryType4
from .component_category_type_5 import ComponentCategoryType5
from .component_category_type_6 import ComponentCategoryType6
from .component_category_type_7 import ComponentCategoryType7
from .component_category_type_8 import ComponentCategoryType8
from .component_category_type_9 import ComponentCategoryType9
from .component_category_type_10 import ComponentCategoryType10
from .component_compatibility import ComponentCompatibility
from .component_origin import ComponentOrigin
from .component_slot import ComponentSlot
from .component_suggested_zone import ComponentSuggestedZone
from .component_template import ComponentTemplate
from .config_document import ConfigDocument
from .config_key_response import ConfigKeyResponse
from .config_key_schema_entry import ConfigKeySchemaEntry
from .config_key_schema_entry_list_item import ConfigKeySchemaEntryListItem
from .config_mutation_response import ConfigMutationResponse
from .control_access import ControlAccess
from .control_action_descriptor import ControlActionDescriptor
from .control_action_descriptor_availability import ControlActionDescriptorAvailability
from .control_action_descriptor_result_type_type_0 import (
    ControlActionDescriptorResultTypeType0,
)
from .control_action_result import ControlActionResult
from .control_action_result_result_type_0 import ControlActionResultResultType0
from .control_action_status import ControlActionStatus
from .control_apply_error import ControlApplyError
from .control_availability_state import ControlAvailabilityState
from .control_binding import ControlBinding
from .control_definition import ControlDefinition
from .control_field_descriptor import ControlFieldDescriptor
from .control_field_descriptor_availability import ControlFieldDescriptorAvailability
from .control_field_descriptor_default_value_type_0 import (
    ControlFieldDescriptorDefaultValueType0,
)
from .control_field_descriptor_value_type import ControlFieldDescriptorValueType
from .control_group_descriptor import ControlGroupDescriptor
from .control_group_kind import ControlGroupKind
from .control_kind_type_0 import ControlKindType0
from .control_kind_type_1 import ControlKindType1
from .control_kind_type_2 import ControlKindType2
from .control_kind_type_3 import ControlKindType3
from .control_kind_type_4 import ControlKindType4
from .control_kind_type_5 import ControlKindType5
from .control_kind_type_6 import ControlKindType6
from .control_kind_type_7 import ControlKindType7
from .control_kind_type_8 import ControlKindType8
from .control_kind_type_9 import ControlKindType9
from .control_object_field import ControlObjectField
from .control_object_field_default_value_type_0 import (
    ControlObjectFieldDefaultValueType0,
)
from .control_object_field_value_type import ControlObjectFieldValueType
from .control_owner import ControlOwner
from .control_persistence import ControlPersistence
from .control_surface_document import ControlSurfaceDocument
from .control_surface_document_values import ControlSurfaceDocumentValues
from .control_surface_list_response import ControlSurfaceListResponse
from .control_surface_scope import ControlSurfaceScope
from .control_type import ControlType
from .control_value import ControlValue
from .control_value_kind import ControlValueKind
from .control_visibility import ControlVisibility
from .corner import Corner
from .create_layer_request import CreateLayerRequest
from .create_layout_request import CreateLayoutRequest
from .create_layout_response_201 import CreateLayoutResponse201
from .create_live_zone_layer_response_201 import CreateLiveZoneLayerResponse201
from .create_live_zone_response_201 import CreateLiveZoneResponse201
from .create_playlist_response_201 import CreatePlaylistResponse201
from .create_preset_response_201 import CreatePresetResponse201
from .create_scene_request import CreateSceneRequest
from .create_scene_response_201 import CreateSceneResponse201
from .create_simulated_display_request import CreateSimulatedDisplayRequest
from .create_simulated_display_response_201 import CreateSimulatedDisplayResponse201
from .create_template_response_201 import CreateTemplateResponse201
from .create_zone_request import CreateZoneRequest
from .deactivate_playlist_response import DeactivatePlaylistResponse
from .deactivate_playlist_response_200 import DeactivatePlaylistResponse200
from .deactivate_scene_response_200 import DeactivateSceneResponse200
from .delete_asset_response import DeleteAssetResponse
from .delete_asset_response_200 import DeleteAssetResponse200
from .delete_attachments_response import DeleteAttachmentsResponse
from .delete_attachments_response_200 import DeleteAttachmentsResponse200
from .delete_config_key_response_200 import DeleteConfigKeyResponse200
from .delete_device_response import DeleteDeviceResponse
from .delete_device_response_200 import DeleteDeviceResponse200
from .delete_display_face_response import DeleteDisplayFaceResponse
from .delete_display_face_response_200 import DeleteDisplayFaceResponse200
from .delete_favorite_response import DeleteFavoriteResponse
from .delete_layout_response import DeleteLayoutResponse
from .delete_layout_response_200 import DeleteLayoutResponse200
from .delete_layout_response_202 import DeleteLayoutResponse202
from .delete_live_zone_layer_response_200 import DeleteLiveZoneLayerResponse200
from .delete_live_zone_response_200 import DeleteLiveZoneResponse200
from .delete_pairing_response import DeletePairingResponse
from .delete_pairing_response_200 import DeletePairingResponse200
from .delete_playlist_response import DeletePlaylistResponse
from .delete_playlist_response_200 import DeletePlaylistResponse200
from .delete_preset_response import DeletePresetResponse
from .delete_preset_response_200 import DeletePresetResponse200
from .delete_scene_response import DeleteSceneResponse
from .delete_scene_response_200 import DeleteSceneResponse200
from .delete_simulated_display_response import DeleteSimulatedDisplayResponse
from .delete_simulated_display_response_200 import DeleteSimulatedDisplayResponse200
from .device_auth_state import DeviceAuthState
from .device_auth_summary import DeviceAuthSummary
from .device_class_hint import DeviceClassHint
from .device_components_response import DeviceComponentsResponse
from .device_components_update_response import DeviceComponentsUpdateResponse
from .device_connection_summary import DeviceConnectionSummary
from .device_origin import DeviceOrigin
from .device_ref import DeviceRef
from .device_summary import DeviceSummary
from .device_summary_list_response import DeviceSummaryListResponse
from .diagnose_check import DiagnoseCheck
from .diagnose_device_output_item import DiagnoseDeviceOutputItem
from .diagnose_device_output_snapshot import DiagnoseDeviceOutputSnapshot
from .diagnose_display_output_snapshot import DiagnoseDisplayOutputSnapshot
from .diagnose_latest_frame_snapshot import DiagnoseLatestFrameSnapshot
from .diagnose_render_snapshot import DiagnoseRenderSnapshot
from .diagnose_render_window_snapshot import DiagnoseRenderWindowSnapshot
from .diagnose_request import DiagnoseRequest
from .diagnose_response import DiagnoseResponse
from .diagnose_snapshot import DiagnoseSnapshot
from .diagnose_summary import DiagnoseSummary
from .diagnose_usb_actor_snapshot import DiagnoseUsbActorSnapshot
from .discover_devices_response_200 import DiscoverDevicesResponse200
from .discover_devices_response_202 import DiscoverDevicesResponse202
from .discover_request import DiscoverRequest
from .discovery_completed_response import DiscoveryCompletedResponse
from .discovery_scan_result import DiscoveryScanResult
from .discovery_scanner_result import DiscoveryScannerResult
from .discovery_started_response import DiscoveryStartedResponse
from .display_class import DisplayClass
from .display_descriptor import DisplayDescriptor
from .display_face_response import DisplayFaceResponse
from .display_face_response_optional import DisplayFaceResponseOptional
from .display_face_response_zone import DisplayFaceResponseZone
from .display_face_scope import DisplayFaceScope
from .display_pixel_format import DisplayPixelFormat
from .display_rect import DisplayRect
from .display_shape import DisplayShape
from .display_summary import DisplaySummary
from .display_summary_list_item import DisplaySummaryListItem
from .driver_capability_set import DriverCapabilitySet
from .driver_config_entry import DriverConfigEntry
from .driver_config_response import DriverConfigResponse
from .driver_list_response import DriverListResponse
from .driver_module_descriptor import DriverModuleDescriptor
from .driver_module_kind import DriverModuleKind
from .driver_presentation import DriverPresentation
from .driver_protocol_descriptor import DriverProtocolDescriptor
from .driver_summary import DriverSummary
from .driver_transport_kind_type_0 import DriverTransportKindType0
from .driver_transport_kind_type_1 import DriverTransportKindType1
from .driver_transport_kind_type_2 import DriverTransportKindType2
from .driver_transport_kind_type_3 import DriverTransportKindType3
from .driver_transport_kind_type_4 import DriverTransportKindType4
from .driver_transport_kind_type_5 import DriverTransportKindType5
from .driver_transport_kind_type_6 import DriverTransportKindType6
from .driver_transport_kind_type_7 import DriverTransportKindType7
from .edge_behavior import EdgeBehavior
from .effect_capability_set import EffectCapabilitySet
from .effect_category import EffectCategory
from .effect_detail_response import EffectDetailResponse
from .effect_health_status import EffectHealthStatus
from .effect_metadata import EffectMetadata
from .effect_playlist import EffectPlaylist
from .effect_playlist_list_response import EffectPlaylistListResponse
from .effect_preset import EffectPreset
from .effect_preset_controls import EffectPresetControls
from .effect_preset_list_response import EffectPresetListResponse
from .effect_preset_origin import EffectPresetOrigin
from .effect_preset_summary import EffectPresetSummary
from .effect_preset_summary_controls import EffectPresetSummaryControls
from .effect_preset_summary_list_response import EffectPresetSummaryListResponse
from .effect_source import EffectSource
from .effect_source_kind import EffectSourceKind
from .effect_summary import EffectSummary
from .effect_summary_list_response import EffectSummaryListResponse
from .favorite_summary import FavoriteSummary
from .favorite_summary_list_response import FavoriteSummaryListResponse
from .fit_mode import FitMode
from .full_frame_copy_session_status import FullFrameCopySessionStatus
from .get_active_layout_response_200 import GetActiveLayoutResponse200
from .get_active_playlist_response_200 import GetActivePlaylistResponse200
from .get_asset_response_200 import GetAssetResponse200
from .get_attachments_response_200 import GetAttachmentsResponse200
from .get_config_key_response_200 import GetConfigKeyResponse200
from .get_config_schema_response_200 import GetConfigSchemaResponse200
from .get_control_surface_response_200 import GetControlSurfaceResponse200
from .get_device_control_surface_response_200 import GetDeviceControlSurfaceResponse200
from .get_device_response_200 import GetDeviceResponse200
from .get_display_face_response_200 import GetDisplayFaceResponse200
from .get_driver_config_response_200 import GetDriverConfigResponse200
from .get_driver_control_surface_response_200 import GetDriverControlSurfaceResponse200
from .get_effect_response_200 import GetEffectResponse200
from .get_layout_response_200 import GetLayoutResponse200
from .get_live_scene_response_200 import GetLiveSceneResponse200
from .get_live_zone_response_200 import GetLiveZoneResponse200
from .get_output_response_200 import GetOutputResponse200
from .get_playlist_response_200 import GetPlaylistResponse200
from .get_preset_response_200 import GetPresetResponse200
from .get_scene_response_200 import GetSceneResponse200
from .get_sensors_response_200 import GetSensorsResponse200
from .get_simulated_display_response_200 import GetSimulatedDisplayResponse200
from .get_system_response_200 import GetSystemResponse200
from .gpu_compositor_probe_status import GpuCompositorProbeStatus
from .gradient_stop import GradientStop
from .health_checks import HealthChecks
from .health_response import HealthResponse
from .identify_attachment_request import IdentifyAttachmentRequest
from .identify_attachment_response import IdentifyAttachmentResponse
from .identify_attachment_response_200 import IdentifyAttachmentResponse200
from .identify_device_response import IdentifyDeviceResponse
from .identify_device_response_200 import IdentifyDeviceResponse200
from .identify_request import IdentifyRequest
from .identify_segment_response import IdentifySegmentResponse
from .identify_segment_response_200 import IdentifySegmentResponse200
from .input_source_issue_status import InputSourceIssueStatus
from .input_source_platform_status_type_0 import InputSourcePlatformStatusType0
from .input_source_platform_status_type_0_type import InputSourcePlatformStatusType0Type
from .input_source_platform_status_type_1 import InputSourcePlatformStatusType1
from .input_source_platform_status_type_1_type import InputSourcePlatformStatusType1Type
from .input_source_status import InputSourceStatus
from .input_status import InputStatus
from .install_effect_response_201 import InstallEffectResponse201
from .installed_effect_response import InstalledEffectResponse
from .invoke_control_action_request import InvokeControlActionRequest
from .invoke_control_surface_action_response_200 import (
    InvokeControlSurfaceActionResponse200,
)
from .latency_histogram_bucket_status import LatencyHistogramBucketStatus
from .latency_histogram_status import LatencyHistogramStatus
from .latency_percentiles_status import LatencyPercentilesStatus
from .latest_frame_status import LatestFrameStatus
from .layer_adjust import LayerAdjust
from .layer_binding import LayerBinding
from .layer_blend_mode import LayerBlendMode
from .layer_parameter import LayerParameter
from .layer_source import LayerSource
from .layer_transform import LayerTransform
from .layout_summary import LayoutSummary
from .layout_summary_list_response import LayoutSummaryListResponse
from .led_topology_type_0 import LedTopologyType0
from .led_topology_type_0_type import LedTopologyType0Type
from .led_topology_type_1 import LedTopologyType1
from .led_topology_type_1_type import LedTopologyType1Type
from .led_topology_type_2 import LedTopologyType2
from .led_topology_type_2_type import LedTopologyType2Type
from .led_topology_type_3 import LedTopologyType3
from .led_topology_type_3_type import LedTopologyType3Type
from .led_topology_type_4 import LedTopologyType4
from .led_topology_type_4_type import LedTopologyType4Type
from .led_topology_type_5 import LedTopologyType5
from .led_topology_type_5_type import LedTopologyType5Type
from .led_topology_type_6 import LedTopologyType6
from .led_topology_type_6_type import LedTopologyType6Type
from .linear_rgba import LinearRgba
from .list_assets_response_200 import ListAssetsResponse200
from .list_audio_devices_response_200 import ListAudioDevicesResponse200
from .list_capture_monitors_response_200 import ListCaptureMonitorsResponse200
from .list_control_surfaces_response_200 import ListControlSurfacesResponse200
from .list_devices_response_200 import ListDevicesResponse200
from .list_displays_response_200 import ListDisplaysResponse200
from .list_drivers_response_200 import ListDriversResponse200
from .list_effect_presets_response_200 import ListEffectPresetsResponse200
from .list_effects_response_200 import ListEffectsResponse200
from .list_favorites_response_200 import ListFavoritesResponse200
from .list_layouts_response_200 import ListLayoutsResponse200
from .list_live_zone_layers_response_200 import ListLiveZoneLayersResponse200
from .list_playlists_response_200 import ListPlaylistsResponse200
from .list_presets_response_200 import ListPresetsResponse200
from .list_scenes_response_200 import ListScenesResponse200
from .list_simulated_displays_response_200 import ListSimulatedDisplaysResponse200
from .list_templates_response_200 import ListTemplatesResponse200
from .live_section import LiveSection
from .loop_mode import LoopMode
from .macos_architecture import MacosArchitecture
from .macos_authorization_state import MacosAuthorizationState
from .macos_capability_owner import MacosCapabilityOwner
from .macos_daemon_handover_phase import MacosDaemonHandoverPhase
from .macos_daemon_owner_conflict_status import MacosDaemonOwnerConflictStatus
from .macos_daemon_owner_recovery_required_status import (
    MacosDaemonOwnerRecoveryRequiredStatus,
)
from .macos_daemon_ownership_status import MacosDaemonOwnershipStatus
from .macos_frame_drop import MacosFrameDrop
from .macos_input_telemetry import MacosInputTelemetry
from .macos_protected_source_state import MacosProtectedSourceState
from .macos_screen_telemetry import MacosScreenTelemetry
from .macos_screen_timing import MacosScreenTiming
from .macos_selection_state_type_0 import MacosSelectionStateType0
from .macos_selection_state_type_0_type import MacosSelectionStateType0Type
from .macos_selection_state_type_1 import MacosSelectionStateType1
from .macos_selection_state_type_1_type import MacosSelectionStateType1Type
from .macos_selection_state_type_2 import MacosSelectionStateType2
from .macos_selection_state_type_2_type import MacosSelectionStateType2Type
from .macos_tahoe_capabilities import MacosTahoeCapabilities
from .macos_tahoe_selection_capabilities import MacosTahoeSelectionCapabilities
from .macos_timing import MacosTiming
from .media_asset_record import MediaAssetRecord
from .media_asset_record_list_response import MediaAssetRecordListResponse
from .media_playback import MediaPlayback
from .member_placement import MemberPlacement
from .normalized_position import NormalizedPosition
from .normalized_rect import NormalizedRect
from .orientation import Orientation
from .output import Output
from .output_component import OutputComponent
from .output_patch_request import OutputPatchRequest
from .output_power_mode import OutputPowerMode
from .output_resource import OutputResource
from .page_info import PageInfo
from .pair_device_response import PairDeviceResponse
from .pair_device_response_200 import PairDeviceResponse200
from .pairing_descriptor import PairingDescriptor
from .pairing_field_descriptor import PairingFieldDescriptor
from .pairing_flow_kind import PairingFlowKind
from .patch_controls_request import PatchControlsRequest
from .patch_controls_request_values import PatchControlsRequestValues
from .patch_display_face_composition_response_200 import (
    PatchDisplayFaceCompositionResponse200,
)
from .patch_display_face_controls_response_200 import (
    PatchDisplayFaceControlsResponse200,
)
from .patch_live_layer_controls_response_200 import PatchLiveLayerControlsResponse200
from .patch_live_scene_response_200 import PatchLiveSceneResponse200
from .patch_live_zone_response_200 import PatchLiveZoneResponse200
from .patch_output_response_200 import PatchOutputResponse200
from .patch_simulated_display_response_200 import PatchSimulatedDisplayResponse200
from .patch_zone_request import PatchZoneRequest
from .playlist_item import PlaylistItem
from .playlist_item_request import PlaylistItemRequest
from .playlist_item_target_type_0 import PlaylistItemTargetType0
from .playlist_item_target_type_0_type import PlaylistItemTargetType0Type
from .playlist_item_target_type_1 import PlaylistItemTargetType1
from .playlist_item_target_type_1_type import PlaylistItemTargetType1Type
from .playlist_target_request_type_0 import PlaylistTargetRequestType0
from .playlist_target_request_type_0_type import PlaylistTargetRequestType0Type
from .playlist_target_request_type_1 import PlaylistTargetRequestType1
from .playlist_target_request_type_1_type import PlaylistTargetRequestType1Type
from .preset_template import PresetTemplate
from .preset_template_controls import PresetTemplateControls
from .preview_demand_status import PreviewDemandStatus
from .preview_layout_response import PreviewLayoutResponse
from .preview_layout_response_200 import PreviewLayoutResponse200
from .preview_runtime_status import PreviewRuntimeStatus
from .preview_source import PreviewSource
from .protected_source_grant_owner import ProtectedSourceGrantOwner
from .put_config_key_response_200 import PutConfigKeyResponse200
from .put_live_zone_layout_response_200 import PutLiveZoneLayoutResponse200
from .redaction import Redaction
from .rejected_control_change import RejectedControlChange
from .rejected_control_change_attempted_value import RejectedControlChangeAttemptedValue
from .remove_favorite_response_200 import RemoveFavoriteResponse200
from .render_acceleration_status import RenderAccelerationStatus
from .render_loop_status import RenderLoopStatus
from .render_surface_status import RenderSurfaceStatus
from .reorder_layers_request import ReorderLayersRequest
from .reorder_live_zone_layers_response_200 import ReorderLiveZoneLayersResponse200
from .replace_live_zone_layer_response_200 import ReplaceLiveZoneLayerResponse200
from .replace_scene_layer_request import ReplaceSceneLayerRequest
from .replace_scene_request import ReplaceSceneRequest
from .replace_scene_request_metadata import ReplaceSceneRequestMetadata
from .replace_scene_request_transition import ReplaceSceneRequestTransition
from .replace_zone_request import ReplaceZoneRequest
from .replace_zone_request_display_target_type_0 import (
    ReplaceZoneRequestDisplayTargetType0,
)
from .rescan_effects_response_200 import RescanEffectsResponse200
from .rescan_response import RescanResponse
from .reset_config_response_200 import ResetConfigResponse200
from .response_meta import ResponseMeta
from .rgb import Rgb
from .rgba import Rgba
from .ring_def import RingDef
from .room_adjacency import RoomAdjacency
from .room_dimensions import RoomDimensions
from .run_diagnostics_response_200 import RunDiagnosticsResponse200
from .sampling_mode_type_0 import SamplingModeType0
from .sampling_mode_type_0_type import SamplingModeType0Type
from .sampling_mode_type_1 import SamplingModeType1
from .sampling_mode_type_1_type import SamplingModeType1Type
from .sampling_mode_type_2 import SamplingModeType2
from .sampling_mode_type_2_type import SamplingModeType2Type
from .sampling_mode_type_3 import SamplingModeType3
from .sampling_mode_type_3_type import SamplingModeType3Type
from .save_playlist_request import SavePlaylistRequest
from .save_preset_request import SavePresetRequest
from .scene_document import SceneDocument
from .scene_document_metadata import SceneDocumentMetadata
from .scene_document_transition import SceneDocumentTransition
from .scene_layer import SceneLayer
from .scene_layer_list_response import SceneLayerListResponse
from .scene_layout_activation_outcome import SceneLayoutActivationOutcome
from .scene_patch_request import ScenePatchRequest
from .scene_summary import SceneSummary
from .scene_summary_list_response import SceneSummaryListResponse
from .screen_capture_capacity_status import ScreenCaptureCapacityStatus
from .segment_summary import SegmentSummary
from .segment_topology_summary_type_0 import SegmentTopologySummaryType0
from .segment_topology_summary_type_0_type import SegmentTopologySummaryType0Type
from .segment_topology_summary_type_1 import SegmentTopologySummaryType1
from .segment_topology_summary_type_1_type import SegmentTopologySummaryType1Type
from .segment_topology_summary_type_2 import SegmentTopologySummaryType2
from .segment_topology_summary_type_2_type import SegmentTopologySummaryType2Type
from .segment_topology_summary_type_3 import SegmentTopologySummaryType3
from .segment_topology_summary_type_3_type import SegmentTopologySummaryType3Type
from .segment_topology_summary_type_4 import SegmentTopologySummaryType4
from .segment_topology_summary_type_4_type import SegmentTopologySummaryType4Type
from .segment_topology_summary_type_5 import SegmentTopologySummaryType5
from .segment_topology_summary_type_5_type import SegmentTopologySummaryType5Type
from .sensor_reading import SensorReading
from .sensor_unit import SensorUnit
from .server_info import ServerInfo
from .session_performance_status import SessionPerformanceStatus
from .set_capture_source_response_200 import SetCaptureSourceResponse200
from .set_display_face_request import SetDisplayFaceRequest
from .set_display_face_request_controls import SetDisplayFaceRequestControls
from .set_display_face_response_200 import SetDisplayFaceResponse200
from .show_config_response_200 import ShowConfigResponse200
from .side_effect_outcome import SideEffectOutcome
from .simulated_display_config import SimulatedDisplayConfig
from .simulated_display_config_list_item import SimulatedDisplayConfigListItem
from .snapshot_scene_request import SnapshotSceneRequest
from .snapshot_scene_response_201 import SnapshotSceneResponse201
from .space_definition import SpaceDefinition
from .spatial_layout import SpatialLayout
from .strip_direction import StripDirection
from .system_resource import SystemResource
from .system_snapshot import SystemSnapshot
from .system_status import SystemStatus
from .template_detail import TemplateDetail
from .template_summary import TemplateSummary
from .template_summary_list_response import TemplateSummaryListResponse
from .time_wave import TimeWave
from .transition_type_type_0 import TransitionTypeType0
from .transition_type_type_0_type import TransitionTypeType0Type
from .unassign_live_zone_member_response_200 import UnassignLiveZoneMemberResponse200
from .update_asset_response_200 import UpdateAssetResponse200
from .update_attachments_request import UpdateAttachmentsRequest
from .update_attachments_response_200 import UpdateAttachmentsResponse200
from .update_device_request import UpdateDeviceRequest
from .update_device_response_200 import UpdateDeviceResponse200
from .update_display_face_composition_request import UpdateDisplayFaceCompositionRequest
from .update_layout_request import UpdateLayoutRequest
from .update_layout_response_200 import UpdateLayoutResponse200
from .update_playlist_response_200 import UpdatePlaylistResponse200
from .update_preset_response_200 import UpdatePresetResponse200
from .update_scene_response_200 import UpdateSceneResponse200
from .update_simulated_display_request import UpdateSimulatedDisplayRequest
from .upload_asset_response_200 import UploadAssetResponse200
from .upload_asset_response_201 import UploadAssetResponse201
from .viewport_rect import ViewportRect
from .wall import Wall
from .web_viewport_render import WebViewportRender
from .winding import Winding
from .zone_layout_request import ZoneLayoutRequest
from .zone_layout_resource import ZoneLayoutResource
from .zone_member import ZoneMember
from .zone_resource import ZoneResource
from .zone_resource_display_target_type_0 import ZoneResourceDisplayTargetType0
from .zone_shape_type_0 import ZoneShapeType0
from .zone_shape_type_0_shape_type import ZoneShapeType0ShapeType
from .zone_shape_type_1 import ZoneShapeType1
from .zone_shape_type_1_shape_type import ZoneShapeType1ShapeType
from .zone_shape_type_2 import ZoneShapeType2
from .zone_shape_type_2_shape_type import ZoneShapeType2ShapeType
from .zone_shape_type_3 import ZoneShapeType3
from .zone_shape_type_3_shape_type import ZoneShapeType3ShapeType

__all__ = (
    "ActionConfirmation",
    "ActionConfirmationLevel",
    "ActivatedSceneRef",
    "ActivatePlaylistResponse",
    "ActivatePlaylistResponse200",
    "ActivateSceneResponse",
    "ActivateSceneResponse200",
    "ActivePlaylistResponse",
    "ActivePlaylistStateResponse",
    "AddFavoriteRequest",
    "AddFavoriteResponse",
    "AddFavoriteResponse200",
    "ApiErrorBody",
    "ApiErrorDetail",
    "AppliedControlChange",
    "AppliedControlChangeValue",
    "ApplyControlChangesResponse",
    "ApplyControlChangesResponseValues",
    "ApplyControlSurfaceValuesResponse200",
    "ApplyEffectPresetResponse200",
    "ApplyEffectRequest",
    "ApplyEffectRequestControlsType0",
    "ApplyEffectResponse",
    "ApplyEffectResponse200",
    "ApplyImpactType0",
    "ApplyImpactType1",
    "ApplyImpactType2",
    "ApplyImpactType3",
    "ApplyImpactType4",
    "ApplyImpactType5",
    "ApplyImpactType6",
    "ApplyImpactType7",
    "ApplyLayoutResponse",
    "ApplyLayoutResponse200",
    "ApplyLayoutResponse202",
    "ApplyPolicyType0",
    "ApplyPolicyType0Kind",
    "ApplyPolicyType1",
    "ApplyPolicyType1Kind",
    "ApplyPolicyType2",
    "ApplyPolicyType2Kind",
    "ApplyPolicyType3",
    "ApplyPolicyType3Kind",
    "ApplyPolicyType4",
    "ApplyPolicyType4Kind",
    "AssetScanStatus",
    "AssetUpdateRequest",
    "AssetUploadResponse",
    "AssetWarningType0",
    "AssetWarningType0Kind",
    "AssetWarningType1",
    "AssetWarningType1Kind",
    "AssignLiveZoneMembersResponse200",
    "AssignMembersRequest",
    "AudioBand",
    "AudioDeviceInfo",
    "AudioDevicesResponse",
    "AuthorizeInputMonitoringResponse200",
    "AuthorizeScreenRecordingResponse200",
    "BindingMap",
    "BindingSourceType0",
    "BindingSourceType0Kind",
    "BindingSourceType1",
    "BindingSourceType1Kind",
    "BindingSourceType2",
    "BindingSourceType2Kind",
    "BindingSourceType3",
    "BindingSourceType3Kind",
    "BTreeMap",
    "BTreeMapAdditionalPropertyType0",
    "BTreeMapAdditionalPropertyType0Kind",
    "BTreeMapAdditionalPropertyType1",
    "BTreeMapAdditionalPropertyType10",
    "BTreeMapAdditionalPropertyType10Kind",
    "BTreeMapAdditionalPropertyType11",
    "BTreeMapAdditionalPropertyType11Kind",
    "BTreeMapAdditionalPropertyType12",
    "BTreeMapAdditionalPropertyType12Kind",
    "BTreeMapAdditionalPropertyType13",
    "BTreeMapAdditionalPropertyType13Kind",
    "BTreeMapAdditionalPropertyType14",
    "BTreeMapAdditionalPropertyType14Kind",
    "BTreeMapAdditionalPropertyType15",
    "BTreeMapAdditionalPropertyType15Kind",
    "BTreeMapAdditionalPropertyType16",
    "BTreeMapAdditionalPropertyType16Kind",
    "BTreeMapAdditionalPropertyType17",
    "BTreeMapAdditionalPropertyType17Kind",
    "BTreeMapAdditionalPropertyType17Value",
    "BTreeMapAdditionalPropertyType18",
    "BTreeMapAdditionalPropertyType18Kind",
    "BTreeMapAdditionalPropertyType1Kind",
    "BTreeMapAdditionalPropertyType2",
    "BTreeMapAdditionalPropertyType2Kind",
    "BTreeMapAdditionalPropertyType3",
    "BTreeMapAdditionalPropertyType3Kind",
    "BTreeMapAdditionalPropertyType4",
    "BTreeMapAdditionalPropertyType4Kind",
    "BTreeMapAdditionalPropertyType5",
    "BTreeMapAdditionalPropertyType5Kind",
    "BTreeMapAdditionalPropertyType6",
    "BTreeMapAdditionalPropertyType6Kind",
    "BTreeMapAdditionalPropertyType7",
    "BTreeMapAdditionalPropertyType7Kind",
    "BTreeMapAdditionalPropertyType8",
    "BTreeMapAdditionalPropertyType8Kind",
    "BTreeMapAdditionalPropertyType9",
    "BTreeMapAdditionalPropertyType9Kind",
    "CaptureAuthorizationResponse",
    "CaptureMonitor",
    "CaptureMonitorListItem",
    "CapturePickerResponse",
    "ClearSceneRequest",
    "ClearSceneResponse200",
    "ComponentBinding",
    "ComponentBindingSummary",
    "ComponentCanvasSize",
    "ComponentCategoryType0",
    "ComponentCategoryType1",
    "ComponentCategoryType10",
    "ComponentCategoryType2",
    "ComponentCategoryType3",
    "ComponentCategoryType4",
    "ComponentCategoryType5",
    "ComponentCategoryType6",
    "ComponentCategoryType7",
    "ComponentCategoryType8",
    "ComponentCategoryType9",
    "ComponentCompatibility",
    "ComponentOrigin",
    "ComponentSlot",
    "ComponentSuggestedZone",
    "ComponentTemplate",
    "ConfigDocument",
    "ConfigKeyResponse",
    "ConfigKeySchemaEntry",
    "ConfigKeySchemaEntryListItem",
    "ConfigMutationResponse",
    "ControlAccess",
    "ControlActionDescriptor",
    "ControlActionDescriptorAvailability",
    "ControlActionDescriptorResultTypeType0",
    "ControlActionResult",
    "ControlActionResultResultType0",
    "ControlActionStatus",
    "ControlApplyError",
    "ControlAvailabilityState",
    "ControlBinding",
    "ControlDefinition",
    "ControlFieldDescriptor",
    "ControlFieldDescriptorAvailability",
    "ControlFieldDescriptorDefaultValueType0",
    "ControlFieldDescriptorValueType",
    "ControlGroupDescriptor",
    "ControlGroupKind",
    "ControlKindType0",
    "ControlKindType1",
    "ControlKindType2",
    "ControlKindType3",
    "ControlKindType4",
    "ControlKindType5",
    "ControlKindType6",
    "ControlKindType7",
    "ControlKindType8",
    "ControlKindType9",
    "ControlObjectField",
    "ControlObjectFieldDefaultValueType0",
    "ControlObjectFieldValueType",
    "ControlOwner",
    "ControlPersistence",
    "ControlSurfaceDocument",
    "ControlSurfaceDocumentValues",
    "ControlSurfaceListResponse",
    "ControlSurfaceScope",
    "ControlType",
    "ControlValue",
    "ControlValueKind",
    "ControlVisibility",
    "Corner",
    "CreateLayerRequest",
    "CreateLayoutRequest",
    "CreateLayoutResponse201",
    "CreateLiveZoneLayerResponse201",
    "CreateLiveZoneResponse201",
    "CreatePlaylistResponse201",
    "CreatePresetResponse201",
    "CreateSceneRequest",
    "CreateSceneResponse201",
    "CreateSimulatedDisplayRequest",
    "CreateSimulatedDisplayResponse201",
    "CreateTemplateResponse201",
    "CreateZoneRequest",
    "DeactivatePlaylistResponse",
    "DeactivatePlaylistResponse200",
    "DeactivateSceneResponse200",
    "DeleteAssetResponse",
    "DeleteAssetResponse200",
    "DeleteAttachmentsResponse",
    "DeleteAttachmentsResponse200",
    "DeleteConfigKeyResponse200",
    "DeleteDeviceResponse",
    "DeleteDeviceResponse200",
    "DeleteDisplayFaceResponse",
    "DeleteDisplayFaceResponse200",
    "DeleteFavoriteResponse",
    "DeleteLayoutResponse",
    "DeleteLayoutResponse200",
    "DeleteLayoutResponse202",
    "DeleteLiveZoneLayerResponse200",
    "DeleteLiveZoneResponse200",
    "DeletePairingResponse",
    "DeletePairingResponse200",
    "DeletePlaylistResponse",
    "DeletePlaylistResponse200",
    "DeletePresetResponse",
    "DeletePresetResponse200",
    "DeleteSceneResponse",
    "DeleteSceneResponse200",
    "DeleteSimulatedDisplayResponse",
    "DeleteSimulatedDisplayResponse200",
    "DeviceAuthState",
    "DeviceAuthSummary",
    "DeviceClassHint",
    "DeviceComponentsResponse",
    "DeviceComponentsUpdateResponse",
    "DeviceConnectionSummary",
    "DeviceOrigin",
    "DeviceRef",
    "DeviceSummary",
    "DeviceSummaryListResponse",
    "DiagnoseCheck",
    "DiagnoseDeviceOutputItem",
    "DiagnoseDeviceOutputSnapshot",
    "DiagnoseDisplayOutputSnapshot",
    "DiagnoseLatestFrameSnapshot",
    "DiagnoseRenderSnapshot",
    "DiagnoseRenderWindowSnapshot",
    "DiagnoseRequest",
    "DiagnoseResponse",
    "DiagnoseSnapshot",
    "DiagnoseSummary",
    "DiagnoseUsbActorSnapshot",
    "DiscoverDevicesResponse200",
    "DiscoverDevicesResponse202",
    "DiscoverRequest",
    "DiscoveryCompletedResponse",
    "DiscoveryScannerResult",
    "DiscoveryScanResult",
    "DiscoveryStartedResponse",
    "DisplayClass",
    "DisplayDescriptor",
    "DisplayFaceResponse",
    "DisplayFaceResponseOptional",
    "DisplayFaceResponseZone",
    "DisplayFaceScope",
    "DisplayPixelFormat",
    "DisplayRect",
    "DisplayShape",
    "DisplaySummary",
    "DisplaySummaryListItem",
    "DriverCapabilitySet",
    "DriverConfigEntry",
    "DriverConfigResponse",
    "DriverListResponse",
    "DriverModuleDescriptor",
    "DriverModuleKind",
    "DriverPresentation",
    "DriverProtocolDescriptor",
    "DriverSummary",
    "DriverTransportKindType0",
    "DriverTransportKindType1",
    "DriverTransportKindType2",
    "DriverTransportKindType3",
    "DriverTransportKindType4",
    "DriverTransportKindType5",
    "DriverTransportKindType6",
    "DriverTransportKindType7",
    "EdgeBehavior",
    "EffectCapabilitySet",
    "EffectCategory",
    "EffectDetailResponse",
    "EffectHealthStatus",
    "EffectMetadata",
    "EffectPlaylist",
    "EffectPlaylistListResponse",
    "EffectPreset",
    "EffectPresetControls",
    "EffectPresetListResponse",
    "EffectPresetOrigin",
    "EffectPresetSummary",
    "EffectPresetSummaryControls",
    "EffectPresetSummaryListResponse",
    "EffectSource",
    "EffectSourceKind",
    "EffectSummary",
    "EffectSummaryListResponse",
    "FavoriteSummary",
    "FavoriteSummaryListResponse",
    "FitMode",
    "FullFrameCopySessionStatus",
    "GetActiveLayoutResponse200",
    "GetActivePlaylistResponse200",
    "GetAssetResponse200",
    "GetAttachmentsResponse200",
    "GetConfigKeyResponse200",
    "GetConfigSchemaResponse200",
    "GetControlSurfaceResponse200",
    "GetDeviceControlSurfaceResponse200",
    "GetDeviceResponse200",
    "GetDisplayFaceResponse200",
    "GetDriverConfigResponse200",
    "GetDriverControlSurfaceResponse200",
    "GetEffectResponse200",
    "GetLayoutResponse200",
    "GetLiveSceneResponse200",
    "GetLiveZoneResponse200",
    "GetOutputResponse200",
    "GetPlaylistResponse200",
    "GetPresetResponse200",
    "GetSceneResponse200",
    "GetSensorsResponse200",
    "GetSimulatedDisplayResponse200",
    "GetSystemResponse200",
    "GpuCompositorProbeStatus",
    "GradientStop",
    "HealthChecks",
    "HealthResponse",
    "IdentifyAttachmentRequest",
    "IdentifyAttachmentResponse",
    "IdentifyAttachmentResponse200",
    "IdentifyDeviceResponse",
    "IdentifyDeviceResponse200",
    "IdentifyRequest",
    "IdentifySegmentResponse",
    "IdentifySegmentResponse200",
    "InputSourceIssueStatus",
    "InputSourcePlatformStatusType0",
    "InputSourcePlatformStatusType0Type",
    "InputSourcePlatformStatusType1",
    "InputSourcePlatformStatusType1Type",
    "InputSourceStatus",
    "InputStatus",
    "InstalledEffectResponse",
    "InstallEffectResponse201",
    "InvokeControlActionRequest",
    "InvokeControlSurfaceActionResponse200",
    "LatencyHistogramBucketStatus",
    "LatencyHistogramStatus",
    "LatencyPercentilesStatus",
    "LatestFrameStatus",
    "LayerAdjust",
    "LayerBinding",
    "LayerBlendMode",
    "LayerParameter",
    "LayerSource",
    "LayerTransform",
    "LayoutSummary",
    "LayoutSummaryListResponse",
    "LedTopologyType0",
    "LedTopologyType0Type",
    "LedTopologyType1",
    "LedTopologyType1Type",
    "LedTopologyType2",
    "LedTopologyType2Type",
    "LedTopologyType3",
    "LedTopologyType3Type",
    "LedTopologyType4",
    "LedTopologyType4Type",
    "LedTopologyType5",
    "LedTopologyType5Type",
    "LedTopologyType6",
    "LedTopologyType6Type",
    "LinearRgba",
    "ListAssetsResponse200",
    "ListAudioDevicesResponse200",
    "ListCaptureMonitorsResponse200",
    "ListControlSurfacesResponse200",
    "ListDevicesResponse200",
    "ListDisplaysResponse200",
    "ListDriversResponse200",
    "ListEffectPresetsResponse200",
    "ListEffectsResponse200",
    "ListFavoritesResponse200",
    "ListLayoutsResponse200",
    "ListLiveZoneLayersResponse200",
    "ListPlaylistsResponse200",
    "ListPresetsResponse200",
    "ListScenesResponse200",
    "ListSimulatedDisplaysResponse200",
    "ListTemplatesResponse200",
    "LiveSection",
    "LoopMode",
    "MacosArchitecture",
    "MacosAuthorizationState",
    "MacosCapabilityOwner",
    "MacosDaemonHandoverPhase",
    "MacosDaemonOwnerConflictStatus",
    "MacosDaemonOwnerRecoveryRequiredStatus",
    "MacosDaemonOwnershipStatus",
    "MacosFrameDrop",
    "MacosInputTelemetry",
    "MacosProtectedSourceState",
    "MacosScreenTelemetry",
    "MacosScreenTiming",
    "MacosSelectionStateType0",
    "MacosSelectionStateType0Type",
    "MacosSelectionStateType1",
    "MacosSelectionStateType1Type",
    "MacosSelectionStateType2",
    "MacosSelectionStateType2Type",
    "MacosTahoeCapabilities",
    "MacosTahoeSelectionCapabilities",
    "MacosTiming",
    "MediaAssetRecord",
    "MediaAssetRecordListResponse",
    "MediaPlayback",
    "MemberPlacement",
    "NormalizedPosition",
    "NormalizedRect",
    "Orientation",
    "Output",
    "OutputComponent",
    "OutputPatchRequest",
    "OutputPowerMode",
    "OutputResource",
    "PageInfo",
    "PairDeviceResponse",
    "PairDeviceResponse200",
    "PairingDescriptor",
    "PairingFieldDescriptor",
    "PairingFlowKind",
    "PatchControlsRequest",
    "PatchControlsRequestValues",
    "PatchDisplayFaceCompositionResponse200",
    "PatchDisplayFaceControlsResponse200",
    "PatchLiveLayerControlsResponse200",
    "PatchLiveSceneResponse200",
    "PatchLiveZoneResponse200",
    "PatchOutputResponse200",
    "PatchSimulatedDisplayResponse200",
    "PatchZoneRequest",
    "PlaylistItem",
    "PlaylistItemRequest",
    "PlaylistItemTargetType0",
    "PlaylistItemTargetType0Type",
    "PlaylistItemTargetType1",
    "PlaylistItemTargetType1Type",
    "PlaylistTargetRequestType0",
    "PlaylistTargetRequestType0Type",
    "PlaylistTargetRequestType1",
    "PlaylistTargetRequestType1Type",
    "PresetTemplate",
    "PresetTemplateControls",
    "PreviewDemandStatus",
    "PreviewLayoutResponse",
    "PreviewLayoutResponse200",
    "PreviewRuntimeStatus",
    "PreviewSource",
    "ProtectedSourceGrantOwner",
    "PutConfigKeyResponse200",
    "PutLiveZoneLayoutResponse200",
    "Redaction",
    "RejectedControlChange",
    "RejectedControlChangeAttemptedValue",
    "RemoveFavoriteResponse200",
    "RenderAccelerationStatus",
    "RenderLoopStatus",
    "RenderSurfaceStatus",
    "ReorderLayersRequest",
    "ReorderLiveZoneLayersResponse200",
    "ReplaceLiveZoneLayerResponse200",
    "ReplaceSceneLayerRequest",
    "ReplaceSceneRequest",
    "ReplaceSceneRequestMetadata",
    "ReplaceSceneRequestTransition",
    "ReplaceZoneRequest",
    "ReplaceZoneRequestDisplayTargetType0",
    "RescanEffectsResponse200",
    "RescanResponse",
    "ResetConfigResponse200",
    "ResponseMeta",
    "Rgb",
    "Rgba",
    "RingDef",
    "RoomAdjacency",
    "RoomDimensions",
    "RunDiagnosticsResponse200",
    "SamplingModeType0",
    "SamplingModeType0Type",
    "SamplingModeType1",
    "SamplingModeType1Type",
    "SamplingModeType2",
    "SamplingModeType2Type",
    "SamplingModeType3",
    "SamplingModeType3Type",
    "SavePlaylistRequest",
    "SavePresetRequest",
    "SceneDocument",
    "SceneDocumentMetadata",
    "SceneDocumentTransition",
    "SceneLayer",
    "SceneLayerListResponse",
    "SceneLayoutActivationOutcome",
    "ScenePatchRequest",
    "SceneSummary",
    "SceneSummaryListResponse",
    "ScreenCaptureCapacityStatus",
    "SegmentSummary",
    "SegmentTopologySummaryType0",
    "SegmentTopologySummaryType0Type",
    "SegmentTopologySummaryType1",
    "SegmentTopologySummaryType1Type",
    "SegmentTopologySummaryType2",
    "SegmentTopologySummaryType2Type",
    "SegmentTopologySummaryType3",
    "SegmentTopologySummaryType3Type",
    "SegmentTopologySummaryType4",
    "SegmentTopologySummaryType4Type",
    "SegmentTopologySummaryType5",
    "SegmentTopologySummaryType5Type",
    "SensorReading",
    "SensorUnit",
    "ServerInfo",
    "SessionPerformanceStatus",
    "SetCaptureSourceResponse200",
    "SetDisplayFaceRequest",
    "SetDisplayFaceRequestControls",
    "SetDisplayFaceResponse200",
    "ShowConfigResponse200",
    "SideEffectOutcome",
    "SimulatedDisplayConfig",
    "SimulatedDisplayConfigListItem",
    "SnapshotSceneRequest",
    "SnapshotSceneResponse201",
    "SpaceDefinition",
    "SpatialLayout",
    "StripDirection",
    "SystemResource",
    "SystemSnapshot",
    "SystemStatus",
    "TemplateDetail",
    "TemplateSummary",
    "TemplateSummaryListResponse",
    "TimeWave",
    "TransitionTypeType0",
    "TransitionTypeType0Type",
    "UnassignLiveZoneMemberResponse200",
    "UpdateAssetResponse200",
    "UpdateAttachmentsRequest",
    "UpdateAttachmentsResponse200",
    "UpdateDeviceRequest",
    "UpdateDeviceResponse200",
    "UpdateDisplayFaceCompositionRequest",
    "UpdateLayoutRequest",
    "UpdateLayoutResponse200",
    "UpdatePlaylistResponse200",
    "UpdatePresetResponse200",
    "UpdateSceneResponse200",
    "UpdateSimulatedDisplayRequest",
    "UploadAssetResponse200",
    "UploadAssetResponse201",
    "ViewportRect",
    "Wall",
    "WebViewportRender",
    "Winding",
    "ZoneLayoutRequest",
    "ZoneLayoutResource",
    "ZoneMember",
    "ZoneResource",
    "ZoneResourceDisplayTargetType0",
    "ZoneShapeType0",
    "ZoneShapeType0ShapeType",
    "ZoneShapeType1",
    "ZoneShapeType1ShapeType",
    "ZoneShapeType2",
    "ZoneShapeType2ShapeType",
    "ZoneShapeType3",
    "ZoneShapeType3ShapeType",
)
